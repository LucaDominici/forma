...
//
// A programme is a git checkout that Forma has something to say about: it has a model, or an issue
// snapshot, or both. Anything else is just a directory, and guessing otherwise would put somebody's
// dotfiles repository into a briefing.
//
...
function findPrograms(dir, depth, out) {
  ...
  if (entries.includes('.git')) {
    ...
    if (existsSync(model) || existsSync(issues)) {
      out.push({ dir, hasModel: existsSync(model) && existsSync(topo), hasIssues: existsSync(issues), ghRepo: ghRepoOf(dir) })
    }
...
if (!found.length) fail(`no programmes under ${ROOT}: a programme is a git checkout carrying ${ARCH}/c4-model.json or ${ARCH}/c4-issues.json.`)
...
for (const program of found) {
  ...
  if (!program.ghRepo) { console.error(`[forma scan] ${id}: no git remote named origin, so ghRepo cannot be derived — add it by hand or the manifest will not validate.`) }
