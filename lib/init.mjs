...
// F12: reject an unknown flag instead of silently ignoring it, and honour `--key=value` as well as
// `--key value` — parseArgs is the ONLY reader of process.argv below.
let values
try {
  ({ values } = parseArgs({ args: process.argv.slice(2), allowPositionals: true, strict: true, options: {
    repo: { type: 'string' }, out: { type: 'string' }, force: { type: 'boolean' },
    include: { type: 'string' }, 'include-tests': { type: 'boolean' }, 'skip-tests': { type: 'boolean' },
  } }))
} catch (e) { console.error(`[forma init] FAIL: ${e.message}`); process.exit(1) }
const REPO = values.repo || process.cwd()
const OUT = values.out || join(REPO, 'docs/architecture/c4-topology.json')
...
const SKIP_TESTS = !values['include-tests'] || !!values['skip-tests']
const KEEP = new Set((values.include || '').split(',').map((s) => s.trim().toLowerCase()).filter(Boolean))
