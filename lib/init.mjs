...
import { goPackages, sourceStat } from './lang.mjs'
...
const sourceFail = (e) => { console.error(`[forma init] ${String((e && e.message) || e)}`); process.exit(1) }
...
;(function walk(dir) {
  ...
  for (const e of ents) {
    ...
    const p = join(dir, e)
    let st; try { st = sourceStat(p) } catch (e2) { sourceFail(e2) }
    if (st.isDirectory()) {
...
for (const name of languages) {
  ...
  if (name === 'Go') {
    let packages
    try { packages = goPackages(REPO, skipDir, !SKIP_TESTS) } catch (e) { sourceFail(e) }
