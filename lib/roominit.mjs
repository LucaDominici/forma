...
function setToday(manifestPath) {
  if (!TODAY) return
...
if (has('--scan')) {
  ...
  if (existsSync(MANIFEST)) setToday(MANIFEST)
...
if (existing) {
  ...
  manifest.programs.push(entry)
}
manifest.programs.sort((a, b) => (a.id < b.id ? -1 : a.id > b.id ? 1 : 0))
writeFileSync(MANIFEST, JSON.stringify(manifest, null, 2) + '\n')

console.log(`[forma room init] ${existing ? 'updated' : 'seeded'} "${id}" in ${rel(MANIFEST)} — ghRepo ${ghRepo || '(unknown: add it by hand)'}, map ${hasMap ? 'yes' : 'no'}.`)
const next = []
