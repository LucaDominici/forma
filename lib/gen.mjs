...
// F12: reject an unknown flag instead of silently ignoring it (a typo used to fall back to a
// default), and honour `--key=value` as well as `--key value` — parseArgs is the ONLY reader of
// process.argv below; there is no second, looser arg()/indexOf parser left to disagree with it.
let values
try {
  ({ values } = parseArgs({ args: process.argv.slice(2), allowPositionals: true, strict: true, options: {
    repo: { type: 'string' }, topology: { type: 'string' }, out: { type: 'string' },
    enrich: { type: 'boolean' }, enricher: { type: 'string' }, 'enrich-model': { type: 'string' },
    status: { type: 'string' }, 'status-apply': { type: 'string' }, 'enrich-apply': { type: 'string' },
    'cluster-min': { type: 'string' }, 'group-min': { type: 'string' },
    'no-cluster': { type: 'boolean' }, 'no-auto-edges': { type: 'boolean' }, 'from-docs': { type: 'boolean' },
  } }))
} catch (e) { console.error(`[gen-c4] FAIL: ${e.message}`); process.exit(1) }
const REPO = values.repo || process.cwd()
const TOPO = values.topology || join(REPO, 'docs/architecture/c4-topology.json')
const OUT = values.out || join(REPO, 'docs/architecture/c4-model.json')
const SCHEMA_VERSION = '1.6.0' // +optional cumulative architecture timeline (additive)
const CLUSTER = !values['no-cluster'] // §2: auto-cluster flat containers; --no-cluster to disable
const ENRICH = !!values.enrich        // §7: opt-in LLM prose for description holes
const ENRICHER = values.enricher || null
const ENRICH_MODEL = values['enrich-model'] || null
const STATUS = values.status || join(REPO, 'docs/architecture/c4-status.json') // curated programme state, optional
const STATUS_SET = values.status !== undefined
const APPLY = values['enrich-apply'] || null          // prose written by the agent driving forma
const STATUS_APPLY = values['status-apply'] || null   // §WP-A7: programme STATE written by that agent
...
const posInt = (key, dflt) => { const raw = values[key] ?? dflt; const v = Number(raw); if (!Number.isInteger(v) || v < 1) fail(`--${key}: expected a positive integer, got "${raw}"`); return v }
...
for (const spec of topo.leafSources) {
  ...
  if (!spec.evidenceOnly) for (const f of files) {
    add({
      ...
      name: basename(f).replace(/\.[a-z0-9]+$/i, ''), status: 'current',
...
if (deadRows.length) fail('docSources contain rows with missing or dead evidence:\n - ' +
...
// In a mixed repo adapters compose: Go packages use imports, while the other stacks still use the
// generic reference pass. `meta.stack` alone is ordered by population, so Go may not be first.
const AUTO_EDGES = !values['no-auto-edges']
const HAS_GO = ((topo.meta && topo.meta.stacks) || []).some((stack) => isGo(stack)) || isGo(topo.meta && topo.meta.stack)
if (AUTO_EDGES && HAS_GO) {
  ...
  const goSources = (topo.leafSources || []).filter((s) => isGo((byId.get(s.parent) || {}).tech))
  const derived = goEdges({ repo: REPO, nodes, edges: topo.edges, includeTests: goSources.some((s) => !s.exclude) })
...
if (AUTO_EDGES) {
  ...
  const srcs = (topo.leafSources || []).filter((s) => byId.has(s.parent) && (!HAS_GO || !isGo(byId.get(s.parent).tech)))
  ...
  for (const s of srcs) {
    ...
    try { for (const f of walk(s)) t += '\n' + readFileSync(join(dir, f), 'utf-8') } catch {}
    text.set(s.parent, t)
  }
