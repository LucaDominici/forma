...
let attachedDocs = []
try { attachedDocs = JSON.parse(readFileSync(OUT, 'utf-8')).source.attachedDocs || [] } catch {}
...
const model = {
  ...
  source: { repo: topo.meta.repo, commit, branch, docPath: topo.docPath, generator: 'gen-c4-model@' + SCHEMA_VERSION, ...(attachedDocs.length ? { attachedDocs } : {}) },
  meta: { ...topo.meta, verifyMethod: 'code+topology (gh re-verify optional)' },
  levels: topo.levels,
  nodes,
