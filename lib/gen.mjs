...
import { isGo, goEdges, sourceStat, confinedSourceRoot } from './lang.mjs'
...
const rp = (p) => { try { return confinedSourceRoot(REPO, join(REPO, p)) } catch (e) { fail((e && e.message) || e) } }
...
function walk(spec) {
  const requested = rp(spec.dir)
  let dir
  try { dir = confinedSourceRoot(REPO, requested) } catch (e) { fail((e && e.message) || e) }
  ...
  ;(function visit(abs, rel) {
    for (const f of readdirSync(abs).sort()) {
      ...
      let stat
      try { stat = sourceStat(child) } catch (e) { fail((e && e.message) || e) }
      if (stat.isDirectory()) {
...
if (topo.docPath) rp(topo.docPath)
for (const n of nodes) for (const e of n.evidence || []) if (['path', 'doc', 'adr', 'test', 'glob'].includes(e.type)) rp(e.ref)
...
let statusPath = null, externalStatus = false
if (existsSync(STATUS) || STATUS_SET) {
  ...
  try { statusPath = relative(REPO, confinedSourceRoot(REPO, STATUS)) } catch { statusPath = null; externalStatus = true }
  console.log(`[gen-c4] status overlay: ${decorated} node(s) decorated from ${statusPath || 'explicit external input (not persisted as a repo path)'}`)
}
...
const AUTO_EDGES = !process.argv.includes('--no-auto-edges')
...
const model = {
  ...
  source: { repo: topo.meta.repo, commit, branch, docPath: topo.docPath, generator: 'gen-c4-model@' + SCHEMA_VERSION, ...(attachedDocs.length ? { attachedDocs } : {}), ...(statusPath ? { statusPath } : {}), ...(externalStatus ? { statusRequired: true } : {}) },
...
if (process.argv.includes('--from-docs')) {
  try {
    ...
    if (existsSync(adrDir)) for (const f of readdirSync(adrDir)) if (/\.md$/.test(f) && !/template/i.test(f)) { adrs++; if (/Status[:*\s]+Accepted/i.test(readFileSync(rp(`docs/adr/${f}`), 'utf-8'))) accepted++ }
