...
import { makeDescribeCtx, resolveDescription, matchFeatureRows } from './describe.mjs'
import { loadCache, mergeCache, enrich, agentPlan, applyFills, statusPlan } from './enrich.mjs'
...
const SCHEMA_VERSION = '1.5.0' // +status2:"unknown", +descSource featurematrix (additive enum value → MINOR)
...
for (const n of nodes) {
  ...
  // catalogue collapse groups childless siblings BY CATEGORY — so 25 distinct leaves collapsed
  // into a single box labelled "container". With the real container, they group the way the
  // feature was written for, or stay apart when they belong apart.
...
const dctx = makeDescribeCtx({ repo: REPO, byId, descriptions: topo.descriptions || {}, docPath: topo.docPath, containerOf, featureDocs: topo.featureDocs || [] })
...
// One validator, two callers: the overlay FILE below, and --status-apply above it. A writer that
// validated differently from the reader could commit a c4-status.json the very next `gen` rejects.
// Returns a list of complaints; the caller decides whether that is a fail or a refusal to write.
function statusPatchErrors(id, patch, resolves) {
  const out = []
  if (!resolves(id)) out.push(`unknown node id "${id}" — it is not in the model (stale overlay?)`)
...
// §WP-A7) --status-apply: the WRITER the overlay never had. `c4-status.json` is validated by a
// complete consumer (below), governed by `check`, and rendered tally-and-all by the viewer — and
// nothing in the repo ever wrote one, so on every repo it stayed absent and every box stayed
// `unknown`. This is the second half of the keyless channel: `--enricher agent` emits the work,
// an agent or a human fills it in, this merges it into the curated file. Same vocabulary, same
// validator, same gate. It writes STATE — never `func`, which belongs to the documents.
if (STATUS_APPLY) {
...
if (ENRICH && ENRICHER === 'agent') {
  ...
  // §WP-A7) the same emit for STATE. Two plans, because the two are not the same question: what a
  // module DOES comes from its docs, what it is WORTH is programme state, and the overlay refuses
  // `func` precisely to keep them apart.
  const spath = join(dirname(OUT), 'status-plan.json')
  const splan = statusPlan(nodes, (n) => matchFeatureRows(n, dctx.featureRows, byId))
  writeFileSync(spath, JSON.stringify({ nodes: splan }, null, 2) + '\n')
  const withDocs = splan.filter((e) => e.documents).length
  console.log(`[gen-c4] --enricher agent: ${splan.length} node(s) with no programme state (${withDocs} carry document evidence) → ${relative(REPO, spath)}, then:`)
  console.log(`[gen-c4]   forma gen --status-apply ${relative(REPO, join(dirname(OUT), 'status-fill.json'))}`)
} else if (ENRICH) {
