...
if (direct) {
  ...
  for (let i = 2; i < process.argv.length; i += 2) {
    ...
    if (!['--repo', '--issues', '--model', '--model-ref', '--topology', '--health', '--findings', '--brief', '--plan', '--apply', '--audit-plan', '--counter-plan', '--today', '--stale-after-days', '--blocked-labels'].includes(flag)) fail(`unknown option: ${flag}`)
