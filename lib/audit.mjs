...
export function makeAuditPlan({ repo, issuesSnapshot, linked, health, model, modelRef, today, staleAfterDays = 14, humanLabels = [], brief = null }) {
  const core = {
    schemaVersion: '0.1',
    today,
    staleAfterDays,
...
export const BRIEF_CAPS = { thesis: 1, risk: 5, decide: 5 }
...
export function briefPlan(brief, issuesSnapshot, repo, { today, staleAfterDays = 14 } = {}) {
  ...
  if (stale.length) prompts.push({ kind: 'stale', prompt: `Re-check these claims: their subject or evidence moved, or they aged out. Rewrite, re-anchor, or drop them by omitting them: ${stale.map((entry) => `${entry.id} (${entry.reason})`).join(', ')}.` })
...
// window, given after the claim was last written. Anything else is "not verified".
...
export function applyBrief(repo, existingBrief, fills, issuesSnapshot, context = {}) {
  ...
  for (const f of fills || []) {
    ...
    if (own(f, 'writtenAt') || own(f, 'evidenceHash') || own(f, 'verified')) throw new Error(`audit apply: ${label} provenance is controlled by forma, not the fill`)
    ...
    if (prior && prior.verified && prior.text === rec.text && prior.evidenceHash === rec.evidenceHash && JSON.stringify(prior.about) === JSON.stringify(rec.about)) rec.verified = prior.verified
...
export function applyCounterResults(repo, issuesSnapshot, _issuesPath, health, findings, plan, rawResult, context = {}) {
  ...
  for (const entry of result.results) {
    ...
    if (entry.claimId.startsWith('brief:')) {
      ...
      brief.claims[index] = { ...brief.claims[index], verified: { verdict: entry.verdict, reason: String(entry.reason).trim(), evidence, at: auditDay(context.today, 'audit counter') } }
...
if (direct) {
  ...
  for (let i = 2; i < process.argv.length; i += 2) {
    ...
    if (!['--repo', '--issues', '--model', '--topology', '--health', '--findings', '--brief', '--plan', '--apply', '--audit-plan', '--counter-plan', '--today', '--stale-after-days', '--blocked-labels'].includes(flag)) fail(`unknown option: ${flag}`)
  ...
  const currentPlan = () => makeAuditPlan({
    repo, issuesSnapshot: issues, linked, health, model, modelRef: relative(repo, paths.model) || 'c4-model.json', today, staleAfterDays, humanLabels, brief,
  })
    ...
    if (fill && Array.isArray(fill.results)) {
      ...
      for (const entry of perEntry.results) {
        attempt('counter', entry.claimId, () => {
          const one = applyCounterResults(repo, issues, paths.issues, nextHealth, nextFindings.findings, plan, { planHash: fill.planHash, results: [entry] }, { ...context, brief: nextBrief })
