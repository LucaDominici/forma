...
export function makeAuditPlan({ repo, issuesSnapshot, linked, health, model, modelRef, today, staleAfterDays = 14, humanLabels = [] }) {
  ...
  const planHash = sha256(JSON.stringify(canonical(core)))
...
export function hashEvidence(repo, evidence, issuesSnapshot, label = 'evidence') {
  ...
  const material = (evidence || []).map((e) => {
    const resolved = validateEvidence(repo, e, label, known)
    if (e.type === 'path') {
...
export function validateCounterResults(plan, result) {
  ...
  return { planHash: result.planHash, results: claims.map((claim) => byId.get(claim.id)) }
}
...
export function applyCounterResults(repo, issuesSnapshot, issuesPath, health, findings, plan, rawResult, context = {}) {
  ...
  if (opts['--plan']) {
    ...
    try {
      if (fill && Array.isArray(fill.results)) {
        const applied = applyCounterResults(repo, issues, paths.issues, health, findings.findings, plan, fill, context)
        ...
        const extraIssues = fill.verdicts.filter((item) => !plannedIssues.has(item.n)).map((item) => item.n)
        const extraDependencies = (fill.dependencyConfirmations || []).filter((item) => !plannedDependencies.has(item.fingerprint)).map((item) => item.fingerprint)
        if (extraIssues.length || extraDependencies.length) fail(`audit fill contains unplanned verdicts or dependency confirmations: ${[...extraIssues, ...extraDependencies].join(', ')}`)
        nextHealth = {
          verdicts: applyVerdicts(repo, health.verdicts, fill.verdicts, known, context).verdicts,
          dependencyConfirmations: applyDependencyConfirmations(repo, health.dependencyConfirmations, fill.dependencyConfirmations || [], issues, context).dependencyConfirmations,
        }
    ...
    console.log(`[forma audit] wrote ${nextHealth.verdicts.length} health verdict(s), ${nextHealth.dependencyConfirmations.length} dependency confirmation(s) and ${nextFindings.findings.length} finding(s)`)
  }
}
