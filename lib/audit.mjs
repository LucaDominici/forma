...
export function validateEvidence(repo, e, label, knownIssues) {
  ...
  if (e.type === 'path') {
    let target
    try { target = confinedSourceRoot(repo, resolve(repo, e.ref)) } catch { throw new Error(`audit apply: ${label} evidence path does not exist in repo: ${e.ref}`) }
    if (!existsSync(target)) throw new Error(`audit apply: ${label} evidence path does not exist in repo: ${e.ref}`)
