...
export const LENSES = [
  ...
  },
  {
    ...
    owns: ['capabilities', 'rtm', 'useCases'],
    publishes: (program) =>
      has((program.derived || {}).rtm) ||
      has((program.derived || {}).capabilities) ||
      has((program.derived || {}).useCases),
  },
  {
    id: 'operations',
    route: 'operations',
    question: 'Can we run it, and survive it failing?',
    // Runbook-to-invariant coverage arrived in wave 8 and lands here, beside the repository's own
    // CI and release signals — which live on the snapshot rather than in `derived`, which is why
    // this lens can publish on signals alone with no derived surface behind it.
    owns: ['runbooks'],
    publishes: (program) => signalsDeclared(program) || has((program.derived || {}).runbooks),
  },
  {
    id: 'provenance',
    route: 'provenance',
...
export function scriptRegions(html) {
  ...
  // Scan from the script body only. A marker is `/*lens:<id>*/` and repository prose reaches the
  // composed artifact verbatim — issue titles, brief claims, embedded canon — so a document that
  // quotes a concrete marker while explaining this rule would otherwise become the FIRST marker,
  // fold the room JSON and the whole spliced hologram into that lens, and turn the gate red for a
  // sentence. Fails closed either way; anchoring means it does not fail at all.
  const at = html.indexOf(SCRIPT_OPENS)
...
export function unpartitionedReads(html) {
  ...
  // BOTH halves, or the head is held to a weaker rule than every region: an alias spelling inside a
  // region is refused by name, and above the partition it would have been invisible — which is a
  // bypass, because a script inserted before the opening one shifts the anchor pair intact.
  const head = derivedTokens(first < 0 ? html : html.slice(0, first))
  for (const key of head.reads) {
    out.push(`derived.${key} is read above the first lens region, where it belongs to no lens`)
  }
  for (const spelling of head.malformed) {
    out.push(`\`${spelling}\` appears above the first lens region, where it belongs to no lens`)
  }
  return out
}

/** The viewer's script must open ON a region, so no viewer code can sit between the two. */
const SCRIPT_OPENS = '"use strict";\n'
 ...
 * @returns {string[]}
 */
function structureViolations(regions, expected) {
  const out = []
...
export function ownershipViolations(html, derivedKeys, lenses = LENSES) {
  ...
  const out = structureViolations(regions, [...declared.keys(), 'shared'])
