...
import { resolveProgramPaths, activePrograms, excludedPrograms, snapshotManifestErrors, arbiterMilestonesPath, loadArbiterMilestones, arbiterUseCasesPath, loadArbiterUseCases, arbiterRunbooksPath, loadArbiterRunbooks } from './roomload.mjs'
  ...
  if (!seam) fail('control-room.html: window.__ROOM__ seam not found — did lib/viewer/control-room.html move?')
    ...
    if (embedded && manifest) {
      ...
      for (const program of composed) {
        ...
        if (!snapshot || !pHealth || !pFindings || (paths.brief !== null && !pBrief)) { portfolioComparable = false; continue }
        ...
        if (snapshotErrors.length) { portfolioComparable = false; continue }
        ...
        // Read it the way the composer does. `room` names the file and exits 1; `check` used to let
        // an ENOENT or a SyntaxError escape as a raw Node stack with no [check-c4] FAIL line — the
        // composer/gate divergence roomload.mjs exists to prevent, on a file roomload introduced.
        ...
        // Same wrapper, same reason: an ENOENT here must name the programme and the file, not
        // escape as a raw Node stack with no [check-c4] FAIL line.
        ...
        const deriveContext = {}
        const fresh = deriveAll({ repo: paths.repo, model: pModel, topo: pTopo, issuesSnapshot: snapshot, health: pHealth, findings: pFindings, brief: pBrief, briefPath: paths.brief, manifest: paths.programManifest, gateInputs: loadedDocs && loadedDocs.gateInputs, arbiterMilestones: pArbiterMilestones, arbiterUseCases: pArbiterUseCases, arbiterRunbooks: pArbiterRunbooks, docs: stripGateInputs(loadedDocs), hasMapDeclared: Boolean(paths.model) }, deriveContext)
