...
// different and lives there). `issues-truncated` and `model-topology-symmetry` are room-only checks
// `check --room` has never run and still does not: `loadProgram` computes them once for room's
// benefit, and this formatter simply has no case for them, so nothing is reported here for either.
...
function checkDiagnosticMessages(d, programId) {
  ...
  if (!what) return [] // issues-truncated, model-topology-symmetry: room-only, not reported here
