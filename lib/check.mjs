...
const DERIVED_LABEL = {
  ...
  queue: 'the prioritized queue', health: 'health staleness', commitDrift: 'commit-drift disclosure', link: 'the issue-to-code link', documentGate: 'document gate',
}
  ...
  if (!seam) fail('control-room.html: window.__ROOM__ seam not found — did lib/viewer/control-room.html move?')
    ...
    if (embedded && manifest) {
      ...
      for (const program of composed) {
        ...
        const fresh = deriveAll({ repo: paths.repo, model: pModel, topo: pTopo, issuesSnapshot: snapshot, health: pHealth, manifest: paths.programManifest, gateInputs: loadedDocs && loadedDocs.gateInputs })
        ...
        if (fresh.documentGate) {
          ...
          for (const finding of fresh.documentGate.findings) {
            evidenceGate(paths.repo, finding.evidence, `DOCUMENT GATE ${program.id} ${finding.id}`, knownIssues)
          }
          for (const claim of fresh.documentGate.claims) {
            evidenceGate(paths.repo, claim.evidence, `DOCUMENT CLAIM ${program.id} ${claim.id}`, knownIssues)
