...
import { activePrograms, excludedPrograms, loadProgram, arbiterMilestonesPath, loadArbiterMilestones, arbiterUseCasesPath, loadArbiterUseCases, arbiterRunbooksPath, loadArbiterRunbooks } from './roomload.mjs'
import { validateEvidence as validateAuditEvidence } from './evidence.mjs'
import { loadDocs, stripGateInputs } from './roomdocs.mjs'
import { parseArgs } from 'node:util'

// F12: reject an unknown flag instead of silently ignoring it. Exits directly rather than through
// `fail` below, which only accumulates into `errs` — an unknown flag must not run the gate at all.
// parseArgs is the ONLY reader of process.argv below, so `--key=value` is honoured too.
let values
try {
  ({ values } = parseArgs({ args: process.argv.slice(2), allowPositionals: true, strict: true, options: {
    repo: { type: 'string' }, model: { type: 'string' }, topology: { type: 'string' },
    issues: { type: 'string' }, health: { type: 'string' }, findings: { type: 'string' },
    room: { type: 'string' }, manifest: { type: 'string' },
  } }))
} catch (e) { console.error(`[check-c4] FAIL: ${e.message}`); process.exit(1) }
const REPO = values.repo || process.cwd()
const MODEL = values.model || join(REPO, 'docs/architecture/c4-model.json')
const TOPO = values.topology || join(REPO, 'docs/architecture/c4-topology.json')
...
// A Control Room over a programme that has no architecture map (issues alone) is gated with
// `--room` from a directory that holds no model. That is not a SKIP of the C4 gate: there is no
// model to be adherent to, and the room block below still runs in full. Without `--room`, a missing
// model is the failure it always was.
const ROOM_ONLY = !existsSync(MODEL) && values.room !== undefined
if (!ROOM_ONLY && !existsSync(MODEL)) { console.error('[check-c4] FAIL: model missing (no SKIP): ' + MODEL); process.exit(1) }
if (!ROOM_ONLY && !existsSync(TOPO)) { console.error('[check-c4] FAIL: topology missing: ' + TOPO); process.exit(1) }
if (ROOM_ONLY) console.log('[check-c4] no model at ' + MODEL + ' — C4 assertions not applicable; gating the Control Room only.')
let model = null, topo = null
if (!ROOM_ONLY) {
const model_ = JSON.parse(readFileSync(MODEL, 'utf-8'))
const topo_ = JSON.parse(readFileSync(TOPO, 'utf-8'))
model = model_; topo = topo_
...
if (topo.sourceCoverage) {
  ...
  ;(function scan(dir, rel, depth) {
    if (depth > 16) return
    let entries; try { entries = readdirSync(dir).sort() } catch (e) { fail(`SOURCE COVERAGE: unreadable directory ${rel || '.'} (${(e && e.code) || e}) — an unscannable directory fails closed, it does not skip.`); return }
    for (const name of entries) {
      if (invisible.has(name) || name.startsWith('.')) continue
      const abs = join(dir, name), key = rel ? rel + '/' + name : name
      let st; try { st = statSync(abs) } catch (e) { fail(`SOURCE COVERAGE: unreadable entry ${key} (${(e && e.code) || e}) — an unstatable entry fails closed, it does not skip.`); continue }
...
const ISSUES = values.issues || rp('docs/architecture/c4-issues.json')
...
if (existsSync(ISSUES)) {
  ...
  if (raw) {
    ...
    if (errs10.length) errs10.forEach((e) => fail('c4-issues.json: ' + e))
      ...
      const cited = new Set()
      for (const n of (model && model.nodes) || []) for (const rawIssue of (n.issues || [])) { const mm = /^#?(\d+)$/.exec(String(rawIssue)); if (mm) cited.add(Number(mm[1])) }
...
const HEALTH = values.health || rp('docs/architecture/c4-health.json')
...
const FINDINGS = values.findings || rp('docs/architecture/c4-findings.json')
...
const ROOM_HTML = values.room || rp('docs/architecture/control-room.html')
const MANIFEST = values.manifest || rp('forma.room.json')
...
// `loadProgram`'s diagnostics, formatted the way `check --room` has always worded them (one line
// per violation, `forma.room.json:` prefixed, "programme" not "program" — room.mjs's own wording is
// different and lives there). `issues-truncated` and `model-topology-symmetry` are checks the
// original inline `check --room` never ran; `loadProgram` marks both `fatal: true` (F8 round 3 —
// a fatal diagnostic with no formatted message meant the programme was silently skipped, no FAIL
// line, no explanation), so both get a message here, worded like room.mjs's own for the same field.
// `model`'s schema case is new (F8); every other case reproduces the exact original message.
const CHECK_FIELD_LABEL = { issues: 'issue snapshot', model: 'model', topology: 'topology', health: 'health overlay', findings: 'findings overlay', brief: 'brief overlay' }
function checkDiagnosticMessages(d, programId) {
  const who = `programme "${programId}"`
  if (d.field === 'manifest-mismatch') return [`forma.room.json: ${who}: ${d.detail}`]
  if (d.field === 'issues-truncated') return [`forma.room.json: ${who}: issue snapshot is truncated; refusing to build counts and proportions from incomplete facts.`]
  if (d.field === 'model-topology-symmetry') return [`forma.room.json: ${who}: model and topology must either both be present or both be absent.`]
  const what = CHECK_FIELD_LABEL[d.field]
  if (!what) return []
  if (d.reason === 'missing') return [`forma.room.json: ${who} ${what} is missing: ${d.path}`]
  if (d.reason === 'invalid-json') return [`forma.room.json: ${who} ${what} is not valid JSON — ${d.detail}`]
  if (d.reason === 'schema') {
    if (d.field === 'model') return d.errors.map((e) => `forma.room.json: ${who} model: ${d.path} fails c4-model.schema.json: ${e}`)
    return d.errors.map((e) => `forma.room.json: ${who} ${what}: ${e}`)
  }
  return []
}
if (existsSync(ROOM_HTML) && existsSync(MANIFEST)) {
  ...
  if (!seam) fail('control-room.html: window.__ROOM__ seam not found — did lib/viewer/control-room.html move?')
    ...
    if (embedded && manifest) {
      ...
      // Re-derived alongside every programme so the loop below can also re-derive the portfolio
      // (F1): a programme skipped by any `continue` here makes the portfolio total incomplete, so
      // one skip disables that cross-programme comparison for this run — the per-programme fail()
      // above already gates the run regardless.
      const loadedPrograms = []
      let portfolioComparable = true
      for (const program of composed) {
        const rendered = embeddedById.get(program.id)
        if (!rendered) { fail(`control-room.html: forma.room.json declares programme "${program.id}", which the artifact does not render — regenerate with \`forma room\`.`); portfolioComparable = false; continue }
        // Same loader `room` uses (F8): reading it any other way is how `check --room` used to
        // accept a hand-edited model.json that `room` would have refused to compose. Diagnostics
        // are reported in full regardless of `fatal` (matching the original, which reported a
        // manifest mismatch or an invalid overlay AND still derived from it) — only a `fatal` one
        // stops this programme from being derived at all (the original's own continuation
        // boundary: an unreadable snapshot/health/findings/declared-brief, or a schema-invalid
        // issue snapshot).
        const { inputs, diagnostics } = loadProgram({ manifestDir, manifest, program, staged: {} })
        for (const d of diagnostics) for (const message of checkDiagnosticMessages(d, program.id)) fail(message)
        if (diagnostics.some((d) => d.fatal)) { portfolioComparable = false; continue }
        const { paths, issuesSnapshot: snapshot, model: pModel, topo: pTopo, health: pHealth, findings: pFindings, brief: pBrief, hasMapDeclared, findingsSchemaOk, briefSchemaOk } = inputs
        ...
        // Read it the way the composer does. `room` names the file and exits 1; `check` lets an
        // ENOENT or a SyntaxError escape as a raw Node stack with no [check-c4] FAIL line — the
        // same as it always has for a malformed arbiter projection.
        const pArbiterMilestones = loadArbiterMilestones(arbiterMilestonesPath(program, paths.repo), (path) => {
          try {
            return JSON.parse(readFileSync(path, 'utf-8'))
          } catch (err) {
            throw new Error(`programme "${program.id}" arbiter milestones: ${path} — ${(err && err.message) || err}`)
          }
        })
        const pArbiterUseCases = loadArbiterUseCases(arbiterUseCasesPath(program, paths.repo), (path) => {
          try {
            return JSON.parse(readFileSync(path, 'utf-8'))
          } catch (err) {
            throw new Error(`programme "${program.id}" arbiter use cases: ${path} — ${(err && err.message) || err}`)
          }
        })
        const pArbiterRunbooks = loadArbiterRunbooks(arbiterRunbooksPath(program, paths.repo), (path) => {
          try {
            return JSON.parse(readFileSync(path, 'utf-8'))
          } catch (err) {
            throw new Error(`programme "${program.id}" arbiter runbooks: ${path} — ${(err && err.message) || err}`)
          }
        })
        // The brief and findings overlays are graded like the other overlays (schema, above), then
        // every claim's/finding's evidence must resolve — an unanchored claim is a fatal read error.
        if (pBrief && briefSchemaOk) for (const claim of pBrief.claims) for (const e of claim.evidence) evidenceGate(paths.repo, e, `forma.room.json: programme "${program.id}" brief claim ${claim.id}`, snapshot)
        if (findingsSchemaOk) for (const finding of pFindings.findings) evidenceGate(paths.repo, finding.evidence, `forma.room.json: programme "${program.id}" finding ${finding.id}`, snapshot)
        const deriveContext = {}
        const fresh = deriveAll({ repo: paths.repo, model: pModel, topo: pTopo, issuesSnapshot: snapshot, health: pHealth, findings: pFindings, brief: pBrief, briefPath: paths.brief, manifest: paths.programManifest, gateInputs: loadedDocs && loadedDocs.gateInputs, arbiterMilestones: pArbiterMilestones, arbiterUseCases: pArbiterUseCases, arbiterRunbooks: pArbiterRunbooks, docs: stripGateInputs(loadedDocs), hasMapDeclared }, deriveContext)
        loadedPrograms.push({ ...program, repo: paths.repo, issuesSnapshot: snapshot, model: pModel, topo: pTopo, hasMap: Boolean(pModel), derived: fresh, deriveContext })
      ...
      // F1: `check` compared each programme's own derived fields but never the cross-programme
      // portfolio aggregate, so a hand-tampered `portfolio.totals` in the embedded __ROOM__ passed
      // unnoticed. Re-derive it the same way `room` does, from the same fresh programme loads.
      if (portfolioComparable) {
        const freshPortfolio = derivePortfolio({ today: manifest.today, programs: loadedPrograms })
        const sameTop = (a, b, label) => { if (JSON.stringify(a) !== JSON.stringify(b)) fail(`control-room.html: portfolio — ${label} does not match a fresh re-derivation — regenerate with \`forma room\`.`) }
        sameTop(freshPortfolio, embedded.portfolio, 'portfolio')
        sameTop(manifest.today, embedded.meta && embedded.meta.today, 'meta.today')
        sameTop(excludedPrograms(manifest).map((p) => ({ id: p.id, ghRepo: p.ghRepo })), embedded.meta && embedded.meta.excluded, 'meta.excluded')
      }
    }
  }
}
...
console.log(ROOM_ONLY ? '[check-c4] OK — the Control Room matches a fresh re-derivation of its inputs (no model to check)' : '[check-c4] OK — model is adherent to src/ (leaves, table count, evidence, planned premises all verified)')
