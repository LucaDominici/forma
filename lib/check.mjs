  ...
  if (!seam) fail('control-room.html: window.__ROOM__ seam not found — did lib/viewer/control-room.html move?')
    ...
    if (embedded && manifest) {
      ...
      for (const program of composed) {
        ...
        const fresh = deriveAll({ repo: paths.repo, model: pModel, topo: pTopo, issuesSnapshot: snapshot, health: pHealth, findings: pFindings, manifest: paths.programManifest, gateInputs: loadedDocs && loadedDocs.gateInputs })
