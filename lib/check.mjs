...
import { activePrograms, excludedPrograms, loadProgram } from './roomload.mjs'
  ...
  if (!seam) fail('control-room.html: window.__ROOM__ seam not found — did lib/viewer/control-room.html move?')
    ...
    if (embedded && manifest) {
      ...
      for (const program of composed) {
        ...
        // accept a hand-edited model.json that `room` would have refused to compose.
        const { inputs, errors } = loadProgram({ manifestDir, manifest, program, staged: {} })
        for (const error of errors) fail(`forma.room.json: ${error}`)
        if (errors.length) { portfolioComparable = false; continue }
        const { paths, issuesSnapshot: snapshot, model: pModel, topo: pTopo, health: pHealth, findings: pFindings, brief: pBrief, docs, gateInputs, arbiterMilestones, arbiterUseCases, arbiterRunbooks, hasMap } = inputs
        ...
        if (pBrief) for (const claim of pBrief.claims) for (const e of claim.evidence) evidenceGate(paths.repo, e, `forma.room.json: programme "${program.id}" brief claim ${claim.id}`, snapshot)
        for (const finding of pFindings.findings) evidenceGate(paths.repo, finding.evidence, `forma.room.json: programme "${program.id}" finding ${finding.id}`, snapshot)
        const deriveContext = {}
        const fresh = deriveAll({ repo: paths.repo, model: pModel, topo: pTopo, issuesSnapshot: snapshot, health: pHealth, findings: pFindings, brief: pBrief, briefPath: paths.brief, manifest: paths.programManifest, gateInputs, arbiterMilestones, arbiterUseCases, arbiterRunbooks, docs, hasMapDeclared: hasMap }, deriveContext)
        loadedPrograms.push({ ...program, repo: paths.repo, issuesSnapshot: snapshot, model: pModel, topo: pTopo, hasMap, derived: fresh, deriveContext })
