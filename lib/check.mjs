    ...
    if (errsH.length) errsH.forEach((e) => fail('c4-health.json: ' + e))
      ...
      for (const v of health.verdicts) {
        ...
        for (const e of v.evidence || []) {
          if (e.type === 'path' && !existsSync(rp(e.ref))) fail(`c4-health.json: #${v.n} evidence path missing: ${e.ref}`)
        }
      }
    }
  }
...
if (existsSync(ROOM_HTML) && existsSync(MANIFEST) && issuesSnapshot) {
  ...
  if (!seam) fail('control-room.html: window.__ROOM__ seam not found — did lib/viewer/control-room.html move?')
    ...
    if (embedded && manifest) {
      const fresh = deriveAll({ repo: REPO, model, topo, issuesSnapshot, healthVerdicts: health.verdicts, manifest })
      const same = (a, b, label) => { if (JSON.stringify(a) !== JSON.stringify(b)) fail(`control-room.html: ${label} does not match a fresh re-derivation — regenerate with \`forma room\`.`) }
      same(fresh.kpis, (embedded.derived || {}).kpis, 'Executive KPIs')
      same(fresh.milestones, (embedded.derived || {}).milestones, 'WBS milestone aggregates')
      same(fresh.kanban, (embedded.derived || {}).kanban, 'Kanban buckets')
      ...
      if (!('commitDrift' in (embedded.derived || {}))) fail('control-room.html: tec must report architecture-layer staleness (commitDrift) — the field is missing entirely.')
      else same(fresh.commitDrift, embedded.derived.commitDrift, 'commit-drift disclosure (tec)')
      ...
      for (const mrow of (embedded.derived || {}).milestones || []) if ('completion' in mrow) fail(`control-room.html: milestone "${mrow.title}" aggregate carries \`completion\` — must stay \`closureRate\`.`)
    }
  }
}
