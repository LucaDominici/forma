...
const DERIVED_LABEL = {
  ...
  queue: 'the prioritized queue', health: 'health staleness', findings: 'findings staleness', brief: 'the brief (claims, staleness, verification)', commitDrift: 'commit-drift disclosure', link: 'the issue-to-code link', documentGate: 'document gate',
}
  ...
  if (!seam) fail('control-room.html: window.__ROOM__ seam not found — did lib/viewer/control-room.html move?')
    ...
    if (embedded && manifest) {
      ...
      for (const program of composed) {
        ...
        const pHealth = readOr(paths.health, 'health overlay', { verdicts: [], dependencyConfirmations: [] })
        const pFindings = readOr(paths.findings, 'findings overlay', { findings: [] })
        const pBrief = paths.brief === null ? null : (existsSync(paths.brief) ? readOr(paths.brief, 'brief overlay', null) : { claims: [] })
        ...
        const fresh = deriveAll({ repo: paths.repo, model: pModel, topo: pTopo, issuesSnapshot: snapshot, health: pHealth, findings: pFindings, brief: pBrief, manifest: paths.programManifest, gateInputs: loadedDocs && loadedDocs.gateInputs })
