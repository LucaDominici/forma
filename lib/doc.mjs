...
const title = `Architecture — ${sys ? sys.name : m.source.repo}`
// Frontmatter is GENERATED, not hand-written: `forma doc` rewrites this whole file, so anything a
// human added here would die at the next run. Emitting it means the scaffold satisfies the doc
// standard by construction and is graded by the same style gate as every hand-authored document —
// rather than being carved out of it, which is how a generated file quietly stops being governed.
// `last_review` is the model's own generation date: the scaffold is exactly as fresh as the model
// it projects, and claiming a review nobody performed would be the false green this repo refuses.
const frontmatter = `---
title: '${title.replace(/'/g, "''")}'
doc_version: '1.0.0'
status: draft
last_review: '${String(m.generatedAt || '').slice(0, 10)}'
owner: ''
canonical_id: 'architecture-scaffold'
tags: ['audience/dev', 'kind/spine']
related: []
---
`
const md = `${frontmatter}<!-- forma: arc42 scaffold projected from c4-model.json. Deterministic sections are generated;
