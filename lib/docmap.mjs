...
// Every pipe table in the text, as {header, rows, lines} of equal width. A malformed row is dropped,
...
export function tables(text) {
  ...
  for (const raw of String(text).replace(/\r\n/g, '\n').split('\n')) {
    ...
    if (line.startsWith('|') && line.endsWith('|') && line.length > 2) {
      ...
      if (!cur) cur = { header: row, rows: [], lines: [] }
      else if (row.length === cur.header.length) { cur.rows.push(row); cur.lines.push(lineNo) }
