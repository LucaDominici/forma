...
//   tuple form), enum, minItems, minProperties, maxProperties, maxLength, minimum, maximum, pattern,
//   anyOf and local $ref.
...
const validateAgainstSchema = (value, schema, path, errs, root) => {
  ...
  if (schema.maxProperties != null && typeIsObject(value) && Object.keys(value).length > schema.maxProperties) errs.push(path + ': expected at most ' + schema.maxProperties + ' property, got ' + Object.keys(value).length)
  // Code points, not UTF-16 units (S2 round-1 MEDIUM-5): an astral character (surrogate pair) is
  // one character to a schema author, not two.
  if ((schema.maxLength != null || schema.minLength != null) && typeof value === 'string') {
    const len = [...value].length
    if (schema.maxLength != null && len > schema.maxLength) errs.push(path + ': expected at most ' + schema.maxLength + ' characters, got ' + len)
    if (schema.minLength != null && len < schema.minLength) errs.push(path + ': expected at least ' + schema.minLength + ' character(s), got ' + len)
  }
...
// The fixed keyword set this file actually enforces (see the header comment). An unlisted keyword
// is never silently ignored: F3 (audit 2026-09-14) found `minLength` ignored for years because
// under-validation had no signal. Walked once per schema load, over schema STRUCTURE only —
// `properties`/`definitions` VALUES are schemas to recurse into, their KEYS are data field names.
const SUPPORTED_KEYWORDS = new Set([
  '$schema', '$id', '$ref', 'title', 'description', 'default', 'format',
  'type', 'required', 'properties', 'additionalProperties', 'items', 'enum',
  'minItems', 'minProperties', 'maxProperties', 'maxLength', 'minLength',
  'minimum', 'maximum', 'pattern', 'anyOf', 'definitions',
])
function unsupportedKeywords(schema, path, errs) {
  if (!schema || typeof schema !== 'object') return
  for (const key of Object.keys(schema)) {
    if (!SUPPORTED_KEYWORDS.has(key)) errs.push(`${path}: unsupported schema keyword "${key}" — validate.mjs enforces only a fixed subset; widen it before using this keyword`)
  }
  if (schema.properties) for (const [k, s] of Object.entries(schema.properties)) unsupportedKeywords(s, `${path}.properties.${k}`, errs)
  if (Array.isArray(schema.items)) errs.push(`${path}: unsupported schema keyword "items" (tuple form) — validate.mjs enforces only a single items schema`)
  else if (schema.items && typeIsObject(schema.items)) unsupportedKeywords(schema.items, `${path}.items`, errs)
  if (typeIsObject(schema.additionalProperties)) unsupportedKeywords(schema.additionalProperties, `${path}.additionalProperties`, errs)
  if (Array.isArray(schema.anyOf)) schema.anyOf.forEach((s, i) => unsupportedKeywords(s, `${path}.anyOf[${i}]`, errs))
  if (schema.definitions) for (const [k, s] of Object.entries(schema.definitions)) unsupportedKeywords(s, `${path}.definitions.${k}`, errs)
}
...
export const validateModel = (model, schemaPath = new URL('./schema/c4-model.schema.json', import.meta.url)) => {
  ...
  const errs = []
  unsupportedKeywords(schema, '<schema>', errs)
