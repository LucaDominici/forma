...
const validateAgainstSchema = (value, schema, path, errs, root) => {
  ...
  if (schema.maxLength != null && typeof value === 'string' && value.length > schema.maxLength) errs.push(path + ': expected at most ' + schema.maxLength + ' characters, got ' + value.length)
  if (schema.minLength != null && typeof value === 'string' && value.length < schema.minLength) errs.push(path + ': expected at least ' + schema.minLength + ' character(s), got ' + value.length)
...
function unsupportedKeywords(schema, path, errs) {
  ...
  if (schema.items && typeIsObject(schema.items)) unsupportedKeywords(schema.items, `${path}.items`, errs)
