...
export function deriveRtm({ repo, rtm, issuesSnapshot }) {
  ...
  if (tracked === null) skipped.push({ path: '*', why: `not a readable git checkout: ${repo}` })
  ...
  return {
    ...
    coverage: { requirements: rows.length, withIssues, pct: rows.length ? Math.round((100 * withIssues) / rows.length) : null },
    progress,
    orphans: {
