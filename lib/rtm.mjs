...
const COLUMNS = {
  ...
  verified: /^(verified[_ ]?by|verification|proof|tests?(?:[_ ]?ref)?)$/i,
...
const refsIn = (cell) => {
  ...
  return [...new Set(raw.filter((token) => token && !/^(?:[-–—]|none|null|n\/?a|tbd)$/i.test(token)))]
}
...
export function parseRequirements(repo, docs, tracked) {
  ...
  for (const entry of [...(docs || [])].sort((a, b) => (a.path < b.path ? -1 : a.path > b.path ? 1 : 0))) {
    ...
    let text
    try { text = readFileSync(confinedSourceRoot(repo, join(repo, rel)), 'utf-8') } catch (e) { const why = String((e && e.message) || e); skipped.push({ path: rel, why: /escapes repository/.test(why) ? 'outside repository' : `unreadable: ${why}` }); continue }
...
export function deriveRtm({ repo, rtm, issuesSnapshot }) {
  ...
  const declaredDocs = new Set((rtm.docs || []).map((entry) => entry && entry.path).filter(Boolean))
  const invalidRequireFrom = [...requireFrom].filter((path) => !declaredDocs.has(path)).sort()
  for (const path of invalidRequireFrom) skipped.push({ path, why: 'requireIssuesFrom path is not declared in rtm.docs' })
  ...
  const scopeComplete = requireFrom.size > 0 && invalidRequireFrom.length === 0
  ...
  const unknown = rows.filter((row) => !row.issuesKnown && !row.verifiedKnown).length
  return {
    requirements: rows.map(({ issuesKnown, verifiedKnown, ...row }) => row).sort((a, b) => sortIds(a.from, b.from) || a.line - b.line),
    byRole: [...rows.reduce((acc, row) => acc.set(row.role, (acc.get(row.role) || 0) + 1), new Map())]
      .sort(([a], [b]) => sortIds(a, b)).map(([role, count]) => ({ role, count })),
    coverage: {
      requirements: rows.length, withIssues, verifiedOnly, accounted, unknown,
      pct: rows.length && unknown === 0 ? Math.round((100 * accounted) / rows.length) : null,
    },
    progress,
    scopeComplete,
    orphans: {
