...
import { trackedFiles } from './repofiles.mjs'

// Re-exported: `parseCapabilityVerification` and callers outside this module (`roominit.mjs`)
// import `trackedFiles` from here. See repofiles.mjs for what it tracks and why.
export { trackedFiles }
...
// The capability ledger needs the execution verdicts already present in an RTM report, but it
// must not pretend that report is a requirements hierarchy. Read only the declared columns and
// join later by an explicit feature/use-case id; no status is inferred from PASS or a test path.
export function parseCapabilityVerification(repo, config, tracked = trackedFiles(repo)) {
  if (!config || !config.path) return { rows: [], skipped: [] }
  const rel = config.path, root = resolve(repo), abs = resolve(root, rel), local = relative(root, abs)
  if (!local || local === '..' || local.startsWith('../')) return { rows: [], skipped: [{ path: rel, why: 'outside repository' }] }
  if (tracked && !tracked.has(rel)) return { rows: [], skipped: [{ path: rel, why: 'not tracked by git' }] }
  let text
  try { text = readFileSync(abs, 'utf-8') } catch (e) { return { rows: [], skipped: [{ path: rel, why: `unreadable: ${(e && e.message) || e}` }] } }
  const columns = {
    feature: /^(feature|feature[_ ]?id)$/i,
    useCase: /^(uc|uc[_ ]?id|use[_ ]?case)$/i,
    verdict: /^(verdict|verdetto|status)$/i,
    execution: /^(execution|esecuzione|result)$/i,
    test: /^(test|test\s*\/\s*journey|verification|proof)$/i,
    requirement: /^(requirement|requisito|req)$/i,
  }
  const rows = []
  for (const table of tables(text)) {
    const iFeature = headerIndex(table.header, columns.feature, config.feature)
    const iUseCase = headerIndex(table.header, columns.useCase, config.useCase)
    const iVerdict = headerIndex(table.header, columns.verdict, config.verdict)
    if ((iFeature < 0 && iUseCase < 0) || iVerdict < 0) continue
    const iExecution = headerIndex(table.header, columns.execution, config.execution)
    const iTest = headerIndex(table.header, columns.test, config.test)
    const iRequirement = headerIndex(table.header, columns.requirement, config.requirement)
    for (let i = 0; i < table.rows.length; i++) {
      if ((table.invalid || [])[i]) continue
      const row = table.rows[i]
      const ids = (at, pattern) => at < 0 ? [] : refsIn(row[at]).map(clean).filter((id) => pattern.test(id))
      const featureIds = ids(iFeature, /^F-[A-Z0-9-]+$/i), useCaseIds = ids(iUseCase, /^UC-[A-Z0-9-]+$/i)
      const verdict = clean(row[iVerdict])
      if ((!featureIds.length && !useCaseIds.length) || !verdict) continue
      rows.push({
        featureIds, useCaseIds, verdict,
        execution: iExecution < 0 ? '' : clean(row[iExecution]),
        test: iTest < 0 ? '' : clean(row[iTest]),
        requirement: iRequirement < 0 ? '' : clean(row[iRequirement]),
        source: { path: rel, line: table.lines[i] },
      })
    }
  }
  return rows.length ? { rows, skipped: [] } : { rows: [], skipped: [{ path: rel, why: 'no feature/use-case plus verdict table' }] }
}
...
export function deriveRtm({ repo, rtm, issuesSnapshot }) {
  ...
  // Only an explicit requireIssuesFrom claim says this matrix is the whole WBS. Auto-discovered
  // tables are useful evidence, but cannot silently turn every open issue into an orphan.
  const scopeComplete = requireFrom.size > 0
  const orphanIssues = scopeComplete ? issues
