...
let countered = 0
if (COUNTER) {
  if (!SKIP_VERIFY && (failedProgrammes.length || skipped)) fail('counter-verification requires a fresh snapshot for every active programme; fix verify or use --skip-verify only with snapshots you already refreshed.')
  const configs = []
  for (const program of active) {
    const p = resolveProgramPaths(dir, manifest, program)
