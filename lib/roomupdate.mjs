...
// Usage: node lib/roomupdate.mjs --manifest <path> [--out <path>] [--gh-cmd <cmd>] [--limit <n>] [--skip-verify]
...
if (!SKIP_VERIFY) {
  for (const program of active) {
    ...
    // `verify` currently needs a model on disk (it decorates nodes as a side effect). A map-less
    // programme therefore cannot refresh its snapshot through verify yet — see the "verify without a
    // model" issue. We skip it loudly instead of failing the whole update.
    if (!p.model) { console.error(`[forma room update] ${program.id}: no model — snapshot refresh needs the "verify without a model" fix; leaving its snapshot as-is.`); skipped++; continue }
    const a = ['--repo', p.repo, '--gh-repo', program.ghRepo, '--issues', p.issues, '--model', p.model, '--gh-cmd', GH_CMD, '--limit', LIMIT]
  ...
  if (failedProgrammes.length) console.error(`[forma room update] verify failed for: ${failedProgrammes.join(', ')} — recomposing with whatever snapshots exist on disk.`)
} else {
...
console.log(`[forma room update] done — ${refreshed} snapshot(s) refreshed${skipped ? `, ${skipped} left as-is` : ''}${failedProgrammes.length ? `, ${failedProgrammes.length} failed` : ''}. Control Room recomposed.`)
