...
// Usage: node lib/roomupdate.mjs --manifest <path> [--out <path>] [--gh-cmd <cmd>] [--skip-verify] [--counter] [--fill] [--author-engine <id>] [--verifier-engine <id>]
...
// F12: reject an unknown flag instead of silently ignoring it — `--limit` used to be parsed and
// forwarded to `verify`, which never read it; it is gone from both ends now. parseArgs is the ONLY
// reader of process.argv below, so `--key=value` is honoured too.
let values
try {
  ({ values } = parseArgs({ args: process.argv.slice(2), allowPositionals: true, strict: true, options: {
    manifest: { type: 'string' }, 'gh-cmd': { type: 'string' }, 'skip-verify': { type: 'boolean' },
    counter: { type: 'boolean' }, fill: { type: 'boolean' }, out: { type: 'string' },
    'author-engine': { type: 'string' }, 'verifier-engine': { type: 'string' },
  } }))
} catch (e) { fail(e.message) }

const MANIFEST = resolve(values.manifest || join(process.cwd(), 'forma.room.json'))
const GH_CMD = values['gh-cmd'] || 'gh'
const SKIP_VERIFY = !!values['skip-verify']
const COUNTER = !!values.counter
const FILL = !!values.fill
const OUT = values.out || null
// `--fill` always re-plans from the CURRENT state before applying (so a stale fill is caught, not
// silently applied), then `--counter` needs a fresh re-plan of its own to name the brief the fill
// just wrote. A counter result can therefore only be written against a plan generated AFTER the
// fill landed and BEFORE this process re-plans again — a plan this command never hands anyone. Ask
// for both in one run and the fill's planHash is already stale by the time the internal re-plan
// runs, or (if the fill did nothing hash-relevant) the counter result still targets a plan this
// process cannot reproduce. Reject the combination up front rather than fail deep inside audit.mjs.
if (FILL && COUNTER) fail('--fill and --counter cannot run together: apply the fill first (`room update --skip-verify --fill --author-engine <id>`), let the verifier counter-verify the regenerated plan, then apply the result (`room update --skip-verify --counter --verifier-engine <id>`).')
// Engine identity (#123): the fill and the counter result are typically written by different
// engines (the writing agent authors the brief, a hostile different engine counter-verifies it).
const AUTHOR_ENGINE = values['author-engine'] || null
const VERIFIER_ENGINE = values['verifier-engine'] || null
...
if (!SKIP_VERIFY) {
  ...
  for (const item of work) {
    ...
    const a = ['--repo', p.repo, '--gh-repo', program.ghRepo, '--issues', item.issues,
      ...(item.model ? ['--model', item.model] : []), '--gh-cmd', GH_CMD,
...
if (COUNTER || FILL) {
  ...
  if (FILL) {
    ...
    for (const { program, p, base, auditPlan } of configs) {
      const applied = spawnSync(process.execPath, [join(HERE, 'audit.mjs'), ...base, '--apply', p.auditFill, '--audit-plan', auditPlan, ...(AUTHOR_ENGINE ? ['--engine', AUTHOR_ENGINE] : [])], { stdio: 'inherit' })
  ...
  for (const { program, p, base, auditPlan } of configs) {
    const applied = spawnSync(process.execPath, [join(HERE, 'audit.mjs'), ...base, '--apply', p.counterResults, '--counter-plan', auditPlan, ...(VERIFIER_ENGINE ? ['--engine', VERIFIER_ENGINE] : [])], { stdio: 'inherit' })
