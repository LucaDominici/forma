...
if (!SKIP_VERIFY) {
  ...
  for (const item of work) {
    ...
    try {
      ...
      item.health = p.health ? stage(p.health, true) : null
    ...
    const a = ['--repo', p.repo, '--gh-repo', program.ghRepo, '--issues', item.issues,
      ...
      ...(item.health ? ['--health', item.health] : []), '--today', p.programManifest.today,
...
if (COUNTER || FILL) {
  ...
  for (const item of work) {
    ...
    try { item.health = item.health || stage(p.health, true); item.findings = stage(p.findings, true); item.auditPlan = stage(p.auditPlan, false) } catch (e) { abort(`${program.id}: could not stage audit overlays — ${(e && e.message) || e}`) }
...
for (const item of work) {
  ...
  if (item.auditPlan) target.auditPlan = item.auditPlan
  if (item.p.auditFill) target.auditFill = item.p.auditFill
  if (item.p.counterResults) target.counterResults = item.p.counterResults
  if (item.p.brief) target.brief = { ...target.brief, path: item.p.brief }
}
