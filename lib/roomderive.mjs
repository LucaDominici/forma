...
import { derivedLenses } from './lenses.mjs'
import { classifyClaimStaleness, classifyDependencyConfirmationStaleness, classifyFindingStaleness, classifyVerdictStaleness, classifyVerification, codepointCompare, evidenceHashOrNull } from './evidence.mjs'
...
export function deriveKpis(issuesSnapshot, linkedCoverage, manifest, healthVerdicts) {
  ...
  // "With problems" (#120 AC1): a fresh (non-stale) health verdict that is not `ok`. A stale verdict
  // kept its text and lost its colour (I8) — it must not still count as a live problem. `undefined`
  // (no health overlay passed at all) stays unmeasured; `[]` (declared, nothing verdicted yet) is a
  // measured zero — the same I6/I7 distinction `linkCoveragePct` already keeps for coverage.
  const freshVerdicts = healthVerdicts ? healthVerdicts.filter((v) => !v.stale) : null
  // "Live resources/deploys" (#120 AC1): the workflow signals already fetched by `forma verify`, not
  // a new probe. Only a workflow that actually ran (has a conclusion) counts toward the denominator;
  // a declared-but-never-run workflow is not observed here either way.
  const workflows = Object.values((issuesSnapshot.signals && issuesSnapshot.signals.workflows) || {})
  const ranWorkflows = workflows.filter((w) => w.state === 'present' && w.conclusion)
  // #120 AC2: the milestone bar's warning line — every milestone in this snapshot is undated, so
  // "ordered by constraint" fell back to title order for all of them and the reader should be told.
  const noMilestoneDueDates = issuesSnapshot.milestones.length > 0 && issuesSnapshot.milestones.every((m) => !m.due)
  return {
    ...
    withProblemsCount: freshVerdicts ? freshVerdicts.filter((v) => v.verdict !== 'ok').length : null,
    withProblemsTotal: freshVerdicts ? freshVerdicts.length : null,
    liveResourcesCount: ranWorkflows.length ? ranWorkflows.filter((w) => w.conclusion === 'success').length : null,
    liveResourcesTotal: ranWorkflows.length || null,
    noMilestoneDueDates,
  }
}
...
export function deriveMilestones(issuesSnapshot) {
  const rows = issuesSnapshot.milestones.map((m) => {
    const total = m.open + m.closed
    // #120 AC2: a milestone's own state, not just its issue counts — closed when it holds issues and
    // none are open, empty when it holds none at all (never invented as "closed").
    const state = total === 0 ? 'empty' : m.open === 0 ? 'closed' : 'open'
    return { ...m, closureRate: total ? Math.round((100 * m.closed) / total) : null, state }
  })
  // Ordered by constraint: a due date is the one real constraint a milestone carries here, so dated
  // milestones sort first by date; undated milestones (the whole set, on viafera today) fall back to
  // title so the order stays deterministic rather than reading as arbitrary.
  return rows.sort((a, b) => {
    if (a.due !== b.due) { if (!a.due) return 1; if (!b.due) return -1; return a.due < b.due ? -1 : 1 }
    return codepointCompare(String(a.title), String(b.title))
  })
}
...
export function deriveDependencies(repo, issuesSnapshot, health = { dependencyConfirmations: [] }) {
  ...
  const confirmed = new Set((health.dependencyConfirmations || []).filter((entry) => {
    const evidenceHash = evidenceHashOrNull(repo, entry.evidence, issuesSnapshot, `dependency ${entry.fingerprint}`)
...
function deriveHealth(repo, issuesSnapshot, health, manifest) {
  const byIssue = new Map((issuesSnapshot.issues || []).map((issue) => [issue.n, issue]))
  const verdicts = (health.verdicts || []).map((verdict) => {
    const evidenceHash = evidenceHashOrNull(repo, verdict.evidence, issuesSnapshot, `health #${verdict.n}`)
...
function deriveFindings(repo, issuesSnapshot, findings, health, manifest) {
  const rows = ((findings && findings.findings) || []).map((finding) => {
    const evidenceHash = evidenceHashOrNull(repo, [finding.evidence], issuesSnapshot, `finding ${finding.id}`)
...
function topoOrder(ids, preds, succs, cmp = numericCmp) {
  const indegree = new Map(ids.map((n) => [n, preds.get(n).length]))
  const ready = ids.filter((n) => indegree.get(n) === 0).sort(cmp)
  const order = []
  while (ready.length) {
    const n = ready.shift()
    order.push(n)
    for (const s of succs.get(n)) {
      ...
      if (indegree.get(s) === 0) { ready.push(s); ready.sort(cmp) }
    }
  }
  return order
}
...
function cycleGroups(ids, succs, scheduled, cmp = numericCmp) {
  const stuck = new Set(ids.filter((n) => !scheduled.includes(n)))
  const groups = [], seen = new Set()
  for (const start of [...stuck].sort(cmp)) {
...
// One CPM pass, shared by the issue-blocking DAG and arbiter's milestone DAG (#133 S4): same Kahn
// order, same forward/backward float sweep, parametrized only by which ids and durations apply and
// which comparator breaks a tie — `a - b` for numeric issue numbers, `codepointCompare` for
// milestone ids, never the other's assumption leaking into the wrong path.
function cpm({ ids, preds, succs, duration, cmp = numericCmp }) {
  const order = topoOrder(ids, preds, succs, cmp)
  const cycles = cycleGroups(ids, succs, order, cmp)
  const scheduled = new Set(order)
  const predsIn = (id) => preds.get(id).filter((p) => scheduled.has(p))
  const succsIn = (id) => succs.get(id).filter((s) => scheduled.has(s))
  ...
  for (const id of [...order].reverse()) {
    const late = succsIn(id).length ? Math.min(...succsIn(id).map((s) => ls.get(s))) : finish
    lf.set(id, late)
    ls.set(id, late - duration(id))
  }
  const rows = new Map(order.map((id) => [id, {
    earlyStart: es.get(id),
    earlyFinish: ef.get(id),
    lateStart: ls.get(id),
    lateFinish: lf.get(id),
    totalFloat: ls.get(id) - es.get(id),
    freeFloat: (succsIn(id).length ? Math.min(...succsIn(id).map((s) => es.get(s))) : finish) - ef.get(id),
    isCritical: ls.get(id) - es.get(id) === 0,
  }]))
  return { order, finish, rows, cycles }
}

// One critical chain, predecessor-first. Ties break on `cmp` (smallest id under `cmp`) so the
// rendered path never flickers between runs on equally-critical arms. `key` is the field that names
// a node on the two node shapes this walks: `n` for issues, `id` for milestones.
function criticalChain(nodes, preds, succs, { key = 'n', cmp = numericCmp } = {}) {
  const byId = new Map(nodes.map((node) => [node[key], node]))
  const critical = (id) => byId.has(id) && byId.get(id).isCritical
  const heads = nodes.filter((node) => node.isCritical && !preds.get(node[key]).some(critical))
  if (!heads.length) return []
  let current = heads.map((node) => node[key]).sort(cmp)[0]
  const chain = [current], guard = new Set([current])
  for (;;) {
    const next = succs.get(current).filter(critical).filter((id) => !guard.has(id)).sort(cmp)[0]
...
/**
 * Use cases, as arbiter measured them (#2480 wave 8).
 *
 * forma DERIVES the join it can see and restates nothing arbiter already decided. Two properties
 * are computed here because they are questions about the SET, which no single row can answer:
 *   - `unexercised` — declared use cases that no tabletop scenario walks. arbiter's gate only fails
 *     a use case CLAIMING `exercised` without a walk; a use case that never made the claim is
 *     silent there and is exactly what a traceability view exists to surface.
 *   - `featureIds` rolled up, so the lens can say how much of the matrix the use cases actually
 *     reach. A matrix row no use case names is not a defect — it is a capability nobody has framed
 *     as something a person wants — but it is worth being able to see.
 *
 * `exercisedBy` is arbiter's measurement and is passed through untouched. Recomputing it here would
 * be forma holding a second opinion about a join arbiter's gate has already proven.
 */
export function deriveUseCases(projection) {
  if (!projection || !Array.isArray(projection.useCases)) return null
  const rows = [...projection.useCases]
    .sort((a, b) => codepointCompare(String(a.id), String(b.id)))
    .map((uc) => ({
      id: String(uc.id),
      actor: String(uc.actor || ''),
      goal: String(uc.goal || ''),
      featureIds: Array.isArray(uc.featureIds) ? uc.featureIds.map(String) : [],
      exercisedBy: Array.isArray(uc.exercisedBy) ? uc.exercisedBy.map(String) : [],
      status: typeof uc.status === 'string' ? uc.status : null,
      prdRef: typeof uc.prdRef === 'string' ? uc.prdRef : null,
    }))
  const features = new Set()
  for (const uc of rows) for (const f of uc.featureIds) features.add(f)
  const unexercised = rows.filter((uc) => uc.exercisedBy.length === 0).map((uc) => uc.id)
  return {
    rows,
    total: rows.length,
    exercised: rows.length - unexercised.length,
    unexercised,
    featuresReached: [...features].sort(),
    actors: [...new Set(rows.map((uc) => uc.actor).filter(Boolean))].sort(),
  }
}

/**
 * Runbook coverage, both halves (#2480 wave 8).
 *
 * The uncovered LIST travels in the projection rather than a count, and it is kept whole here for
 * the reason arbiter wrote it that way: an operations view whose only signal is "49" cannot tell an
 * operator which failure they have no procedure for. `orphans` is the other direction — a runbook
 * handling nothing — which arbiter's gate already fails on, so it should always be empty; it is
 * derived anyway because a surface that can only display agreement cannot show a disagreement.
 */
export function deriveRunbooks(projection) {
  if (!projection || !Array.isArray(projection.runbooks)) return null
  const rows = [...projection.runbooks]
    .sort((a, b) => codepointCompare(String(a.id), String(b.id)))
    .map((rb) => ({
      id: String(rb.id),
      file: String(rb.file || ''),
      handles: Array.isArray(rb.handles) ? rb.handles.map(String) : [],
    }))
  const coverage = projection.coverage || {}
  // Sorted here even though arbiter's projection already sorts it, for the same reason `rows` is:
  // a display whose order depends on the producer's ordering guarantee is unstable the day that
  // guarantee changes, and sorting one collection but not the other is the inconsistency that
  // makes the dependency invisible.
  const uncovered = (Array.isArray(coverage.uncovered) ? coverage.uncovered.map(String) : []).sort()
  const operationalTotal =
    typeof coverage.operationalTotal === 'number' ? coverage.operationalTotal : null
  return {
    rows,
    total: rows.length,
    handled: [...new Set(rows.flatMap((rb) => rb.handles))].sort(),
    uncovered,
    operationalTotal,
    // Null rather than 0 when arbiter declared no total: "no procedures at all" and "the projection
    // did not say" are different claims, and a percentage invented from the second is a lie a
    // reader cannot detect.
    coveredCount: operationalTotal === null ? null : operationalTotal - uncovered.length,
    orphans: rows.filter((rb) => rb.handles.length === 0).map((rb) => rb.id),
  }
}
...
//
// Ids are strings, not issue numbers, so every ordering here — declaration, dependency lists,
// ready-queue ties, the chain walk — goes through `codepointCompare` (#133 S4): the one CPM core
// shared with the issue path takes a `cmp`, and a milestone id is never handed the issue path's
// `a - b`, which is NaN on a string and would silently fall back to insertion order.
export function deriveMilestonePath(projection) {
  if (!projection || !Array.isArray(projection.milestones)) return null
  const rows = [...projection.milestones].sort((a, b) => codepointCompare(String(a.id), String(b.id)))
  const ids = rows.map((m) => String(m.id))
  const known = new Set(ids)
  const preds = new Map(ids.map((id) => [id, []])), succs = new Map(ids.map((id) => [id, []]))
  for (const m of rows) {
    for (const dep of (m.depends_on || []).map(String).sort(codepointCompare)) {
  ...
  const { order, finish, rows: cpmRows, cycles } = cpm({ ids, preds, succs, duration, cmp: codepointCompare })
  const unestimated = ids.filter((id) => estimate.get(id) === null && order.includes(id))
  const nodes = [...order].sort(codepointCompare).map((id) => ({
    id,
  ...
  return {
    ...
    unestimated,
    nodes,
    criticalPath: criticalChain(nodes, preds, succs, { key: 'id', cmp: codepointCompare }),
    cycles,
  }
}
...
export function deriveMilestoneReconciliation(projection, issuesSnapshot) {
  ...
  const drift = []
  for (const m of [...projection.milestones].sort((a, b) => codepointCompare(String(a.id), String(b.id)))) {
...
export function deriveCriticalPath(dependencies, issuesSnapshot) {
  ...
  const { order, finish, rows, cycles } = cpm({ ids, preds, succs, duration, cmp: numericCmp })
  const nodes = [...order].sort(numericCmp).map((n) => ({
    n,
    duration: duration(n),
    ...rows.get(n),
  }))
  return {
    ...
    projectDurationDays: finish,
    nodes,
    criticalPath: criticalChain(nodes, preds, succs, { key: 'n', cmp: numericCmp }),
    cycles,
    excludedForeign,
  }
}

// "Why this batch" is not a curated blocks file (SCOPE-room §4): it is whatever the brief already
// says about this block's milestone or one of its issues — a typed `note` claim with the same
// anchor, staleness and refusal path as every other claim, so the narrative rots in the open. It is
// attached HERE rather than read off derived.brief in the viewer, because the verdict lens owns the
// brief and a second reader would give that surface a second home (I20).
export function deriveBlocks(issuesSnapshot, queue, dependencies, manifest, briefDerived) {
  ...
  return (queue.clusters || []).map((cluster, index) => {
    ...
    return {
      ...
      notes: ((briefDerived || {}).notes || []).filter((claim) => claim.about && (
        (milestones.length === 1 && claim.about.milestone === milestones[0]) ||
        (claim.about.issue != null && numbers.includes(claim.about.issue))
      )),
    }
...
export function deriveCapabilities(repo, config, issuesSnapshot, dependencies) {
  ...
  return {
    ...
    // The ledger carries its own uncertainty. Without this the viewer had to reach into
    // derived.dependencies to tell "nothing blocks this" from "we cannot see what blocks this"
    // (I6/I7) — a second surface read from a third lens, which is exactly what I20 refuses.
    dependenciesComplete: dependencies.supported === true && dependencies.complete === true,
  }
}
...
// `byIssue` is null when the git history behind it is unreadable (`linked.error`) — the checkpoint's
// own nodes come from the timeline patch, not git, and stay known; but which issues reached them is
// unmeasured, and reports null throughout rather than the "0 of 0" an empty map would fake (I6).
function deriveCheckpoints(model, issuesSnapshot, byIssue) {
  ...
  return timeline.checkpoints.map((cp) => {
    ...
    if (!byIssue) return { id: cp.id, label: cp.label, nodes: [...nodes].sort(), issues: null, closed: null, total: null, pct: null }
    const issues = []
    for (const [n, ids] of byIssue) if ([...ids].some((id) => nodes.has(id)) && stateByIssue.has(n)) issues.push(n)
    issues.sort((a, b) => a - b)
...
function deriveCommitDrift(repo, sourceCommit) {
...
export function deriveAll({ repo, model, topo, issuesSnapshot, health, findings, brief, briefPath, manifest, gateInputs, arbiterMilestones, arbiterUseCases, arbiterRunbooks, docs, hasMapDeclared }, context) {
  ...
  const linked = linkIssuesToNodes(repo, hasMap ? model : { nodes: [] }, hasMap ? topo : null, { linkMaxFiles: manifest.linkMaxFiles || 50 })
  if (context) context.linked = linked
  // A repo that is not a git checkout (no history to read) is not the same claim as "read the
  // history and found nothing" (I6) — `linked.error` set means the coverage below is unmeasured,
  // never a measured zero.
  const coverage = hasMap && !linked.error ? coverageOf(linked.byIssue, issuesSnapshot.issues.map((it) => it.n)) : null
  const healthState = health || { verdicts: [], dependencyConfirmations: [] }
  ...
  const briefDerived = deriveBrief(repo, issuesSnapshot, brief || null, manifest)
  const derived = {
    kpis: deriveKpis(issuesSnapshot, coverage, manifest, healthDerived.verdicts),
    ...
    useCases: deriveUseCases(arbiterUseCases),
    runbooks: deriveRunbooks(arbiterRunbooks),
    milestoneReconciliation: deriveMilestoneReconciliation(arbiterMilestones, issuesSnapshot),
    blocks: deriveBlocks(issuesSnapshot, queue, dependencies, manifest, briefDerived),
    ...
    checkpoints: hasMap ? deriveCheckpoints(model, issuesSnapshot, linked.error ? null : linked.byIssue) : null,
    link: hasMap ? {
  ...
  // Which of the six lenses this programme can publish, decided by the declared table in
  // lenses.mjs over the artifacts that actually exist. Derived rather than composed so `check`
  // re-derives it like every other aggregate: a briefing whose routing was edited by hand to mount
  // a lens its artifacts do not support fails the gate naming the lens (I20, I7).
  // `hasMap` as the BRIEFING declares it, not as this function happens to compute it. The viewer
  // reads program.hasMap; if the publication predicate read a different one the two could disagree
  // and the architecture lens would mount over a model the frame cannot draw.
  return {
    ...derived,
    lenses: derivedLenses({ hasMap: hasMapDeclared === undefined ? hasMap : hasMapDeclared, docs, issuesSnapshot, derived }),
  }
}
...
export function derivePortfolio({ today, programs }) {
  ...
  for (const program of programs) {
    ...
    // Two sources, never fused without distinction: a declared label, or an open blocker at the other
    // end of an active dependency edge (native, or prose the agent confirmed). The row says which.
    const activeEdges = (dependencyState.edges || []).filter((edge) => edge.active && edge.from.repo === program.ghRepo)
    const blockersOf = (n) => activeEdges.filter((edge) => edge.from.number === n).map((edge) => ({ repo: edge.to.repo, number: edge.to.number, source: edge.source })).sort((a, b) => a.number - b.number)
    const labelsOf = (it) => (it.labels || []).filter((label) => blockedLabelSet.has(label)).sort()
    const blockedByLabel = blockedIssues.filter((it) => labelsOf(it).length).length
    const blockedByDependency = blockedIssues.filter((it) => dependencyBlocked.has(it.n)).length
    ...
    // Unmeasured (null) when the git history behind the link is unreadable — an all-zero object
    // would read as "this node has no work", a measured claim nobody made (I6).
    const workPerNode = hasMap && !linked.error ? Object.fromEntries((program.model.nodes || []).map((node) => [node.id, 0])) : null
    const workPerNodeOpen = hasMap && !linked.error ? Object.fromEntries((program.model.nodes || []).map((node) => [node.id, 0])) : null
    ...
    for (const issue of blockedIssues) {
      ...
      blocked.push({
        item: {
          ...
          blockedBy: { labels: labelsOf(issue), blockers: blockersOf(issue.n) },
          verdict: verdict ? verdict.verdict : null, why: verdict ? verdict.why : null,
          evidence: verdict ? verdict.evidence : null,
          // null (unmeasured) when the map exists but its git history does not — distinct from the
          // no-map case, where "which nodes" does not apply and [] is the honest answer, not a gap.
          nodes: hasMap ? (linked.error ? null : [...(linked.byIssue.get(issue.n) || [])].sort()) : [],
          ...
          // Carried on the item so the viewer never reaches into program.derived.link.error itself
          // (same reasoning as dependenciesComplete above) — an unreadable history must not render
          // `lastLanded`/`daysSinceLastLanding`'s null as "never landed", which is a claim, not a gap.
          landingMeasured: !linked.error,
          priority,
        },
    ...
    let cumulative = 0
    // null (unmeasured) when the git history is unreadable — an empty series would read as "measured
    // and nothing landed", indistinguishable from a repo that genuinely has no linked commits (I6).
    landing.push({ program: program.id, months: linked.error ? null : [...months].sort(([a], [b]) => a < b ? -1 : a > b ? 1 : 0).map(([month, landed]) => ({ month, landed, cumulative: cumulative += landed })) })
    moving.push({
      program: program.id,
    ...
    summaries.push({
      ...
      blockedByLabel: complete ? blockedByLabel : null,
      blockedByDependency: complete ? blockedByDependency : null,
      snapshotAgeDays: daysBetween(program.issuesSnapshot.fetchedAt, today),
      milestones: deriveMilestones(program.issuesSnapshot),
      workPerNode,
      workPerNodeOpen,
      // A repo with no readable history is unmeasured, not a measured 0% — same guard as
      // deriveAll's per-programme `coverage` (I6, S2 round-1 HIGH-1).
      linkCoverage: hasMap && !linked.error ? coverageOf(linked.byIssue, issues.map((it) => it.n)) : null,
