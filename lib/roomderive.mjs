...
export function derivePortfolio({ today, programs, linkMaxFiles }) {
  ...
  for (const program of programs) {
    ...
    moving.push({
      ...
      count: declared ? movingIssues.length : null,
      byCluster: declared ? queue.clusters.map((cluster) => ({ ...cluster, issues: cluster.issues.filter((n) => movingNumbers.has(n)) })).filter((cluster) => cluster.issues.length) : null,
    })
    summaries.push({
      id: program.id,
      ...
      blockedRule: { declared, inert: declared && blockedLabels.length === 0, labels: blockedLabels },
      blocked: declared ? blockedIssues.length : null,
