...
export function deriveKpis(issuesSnapshot, linkedCoverage, manifest) {
  ...
  return {
    ...
    linkCoveragePct: linkedCoverage.pct,
  }
}
...
export function deriveAll({ repo, model, topo, issuesSnapshot, healthVerdicts, manifest }) {
  const linked = deriveLink(repo, model, topo, manifest)
  const coverage = coverageOf(linked.byIssue, issuesSnapshot.issues.map((it) => it.n))
  return {
    ...
    commitDrift: deriveCommitDrift(repo, model.source && model.source.commit),
    link: {
...
export function derivePortfolio({ today, programs }) {
  ...
  for (const program of programs) {
    ...
    const linked = deriveLink(program.repo, hasMap ? program.model : { nodes: [] }, hasMap ? program.topo : null, {})
    ...
    for (const dates of linked.datesByIssue.values()) {
