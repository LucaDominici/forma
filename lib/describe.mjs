...
function readmeFirstPara(repo, dir, cache) {
  if (cache.has(dir)) return cache.get(dir)
  let out = null
  try { const p = confinedSourceRoot(repo, join(dir, 'README.md')); if (existsSync(p)) out = firstPara(readFileSync(p, 'utf-8')) } catch {}
  cache.set(dir, out)
  return out
}
...
function buildArc42Index(repo, docPath) {
  ...
  let text; try { text = readFileSync(confinedSourceRoot(repo, join(repo, docPath)), 'utf-8') } catch { return idx }
...
export function resolveDescription(node, ctx) {
  ...
  if (pth) {
    let abs
    try { abs = confinedSourceRoot(ctx.repo, join(ctx.repo, pth.ref)) } catch { abs = null }
    if (!abs) return { func: measuredFunc(node, ctx) || '', descSource: 'fallback' }
    const ds = moduleDocstring(abs)
    if (ds) return { func: ds, descSource: 'docstring' }
    const rd = readmeFirstPara(ctx.repo, ownDir(abs), ctx.readmeCache)
  ...
  if (!pth && glb) {
    let dir
    try { dir = confinedSourceRoot(ctx.repo, join(ctx.repo, glb.ref)) } catch { dir = null }
    const rd = dir && readmeFirstPara(ctx.repo, dir, ctx.readmeCache)
    if (rd) return { func: rd, descSource: 'readme' }
  }
