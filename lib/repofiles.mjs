// repofiles.mjs — `git ls-files`, read once per repo per process (#133 S4). docmap.mjs, roomdocs.mjs
// and rtm.mjs each shelled out to git independently for the same answer; one `forma room` compose
// paid that cost three times over for a file list that cannot change mid-process. Memoised here so
// every caller shares one spawn and one Set, never a second opinion about which files git tracks.
//
