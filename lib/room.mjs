...
const loadedPrograms = active.map((program) => {
  ...
  // `paths` remain the durable, manifest-declared locations. `inputs` may be transaction-local
  // copies written by room update; only reads use them, so provenance (notably brief.previous)
  // never points at a file that disappears after publication.
  const inputs = { ...paths, ...(stagedInputs[program.id] || {}) }
  const repo = paths.repo
  const issuesSnapshot = readValidated(inputs.issues, 'c4-issues.schema.json', `${label} issue snapshot`)
  ...
  if (hasMap) {
    model = readJson(inputs.model, `${label} model`)
    const modelErrs = validateModel(model)
    if (modelErrs.length) fail(`${label} model: ${paths.model} fails c4-model.schema.json:\n - ` + modelErrs.join('\n - '))
    topo = readJson(inputs.topology, `${label} topology`)
  }
  // A declared overlay that does not exist yet is the state before the first `audit --apply`:
  // empty, not an error. An overlay that exists and does not validate stays fatal.
  const overlay = (path, schemaName, what, empty) => (path === null || !existsSync(path)) ? empty : readValidated(path, schemaName, `${label} ${what}`)
  const health = overlay(inputs.health, 'c4-health.schema.json', 'health overlay', { verdicts: [], dependencyConfirmations: [] })
  const findings = overlay(inputs.findings, 'c4-findings.schema.json', 'findings overlay', { findings: [] })
  ...
  const brief = paths.brief === null ? null : overlay(inputs.brief, 'c4-brief.schema.json', 'brief overlay', { claims: [] })
  ...
  const derived = deriveAll({ repo, model, topo, issuesSnapshot, health, findings, brief, briefPath: paths.brief, manifest: paths.programManifest, gateInputs, arbiterMilestones, arbiterUseCases, arbiterRunbooks, docs, hasMapDeclared: hasMap }, deriveContext)
