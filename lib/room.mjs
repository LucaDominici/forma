...
import { resolveProgramPaths, activePrograms, excludedPrograms } from './roomload.mjs'
...
const loadedPrograms = active.map((program) => {
  ...
  // Documents are carried, not derived: `check` re-derives aggregates, and re-reading a repository's
  // whole prose corpus on every gate run would make the gate cost what the composer costs.
  const docs = program.docs === undefined
    ? null
    : loadDocs(repo, { include: program.docs.include, canon: program.docs.canon, maxBytes: (manifest.docs || {}).maxBytes })
  return { ...program, repo, issuesSnapshot, model, topo, health, findings, hasMap, derived, docs }
})
...
for (const seam of ['/*__ROOM_JSON__*/null', '/*__STRINGS__*/null', '<!--__HOLO_SRC__-->']) {
  if (!out.includes(seam)) fail(`template seam ${seam} not found in control-room.html — did it move?`)
}
out = out.replace('/*__ROOM_JSON__*/null', roomJson).replace('/*__STRINGS__*/null', stringsJson).replace('<!--__HOLO_SRC__-->', viewerHtml)
...
function serve() {
  ...
  const server = createServer((req, res) => {
    ...
    if (req.method === 'GET' && (req.url === '/' || req.url.startsWith('/#') || req.url.startsWith('/?'))) {
      try { return send(200, 'text/html; charset=utf-8', compose().html) } catch (e) { return send(500, 'text/plain', String((e && e.message) || e)) }
    }
