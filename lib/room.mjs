...
// Usage: node lib/room.mjs --manifest <path> [--out <path>] [--staged-inputs <path>]
...
import { resetTrackedFiles } from './repofiles.mjs'

// The exact wording `room` has always failed with, now driven off `loadProgram`'s structured
// diagnostics instead of inline fail() calls — `room` still fails on the very FIRST one, in the
// same order the original inline code produced them, so a caller sees the identical message it
// always did for whichever check trips first.
const ROOM_FIELD_LABEL = { issues: 'issue snapshot', model: 'model', topology: 'topology', health: 'health overlay', findings: 'findings overlay', brief: 'brief overlay' }
const ROOM_FIELD_SCHEMA = { issues: 'c4-issues.schema.json', model: 'c4-model.schema.json', health: 'c4-health.schema.json', findings: 'c4-findings.schema.json', brief: 'c4-brief.schema.json' }
function formatRoomDiagnostic(d, programId) {
  const label = `program "${programId}"`
  if (d.field === 'issues-truncated') return `${label}: issue snapshot is truncated; refusing to build counts and proportions from incomplete facts.`
  if (d.field === 'manifest-mismatch') return `${label}: ${d.detail} — regenerate the snapshot or fix the manifest.`
  if (d.field === 'model-topology-symmetry') return `${label}: model and topology must either both be present or both be absent.`
  const what = ROOM_FIELD_LABEL[d.field] || d.field
  if (d.reason === 'schema') return `${label} ${what}: ${d.path} fails ${ROOM_FIELD_SCHEMA[d.field]}:\n - ` + d.errors.join('\n - ')
  return `${label} ${what}: ${d.path} — ${d.detail}`
}
...
for (let i = 2; i < process.argv.length; i += 1) {
  ...
  if (flag !== '--manifest' && flag !== '--out' && flag !== '--port' && flag !== '--staged-inputs') fail(`unknown option: ${flag}`)
...
const stagedInputs = options['--staged-inputs'] ? readJson(resolve(options['--staged-inputs']), 'staged inputs') : {}
if (!stagedInputs || Array.isArray(stagedInputs) || typeof stagedInputs !== 'object') fail('staged inputs must be an object keyed by programme id')
for (const [id, inputs] of Object.entries(stagedInputs)) {
  if (!inputs || Array.isArray(inputs) || typeof inputs !== 'object') fail(`staged inputs for ${id} must be an object`)
  for (const [key, path] of Object.entries(inputs)) if (!['issues', 'model', 'health', 'findings', 'brief'].includes(key) || typeof path !== 'string') fail(`staged inputs for ${id} contain an invalid path`)
}
...
// One `git ls-files` per repo per COMPOSE, not per process (#133 S4 round 2): `--serve` calls
// `compose()` again on every GET, and a file added between two requests must be visible on the
// next one.
resetTrackedFiles()
...
const loadedPrograms = active.map((program) => {
  // Same loader `check` uses, so the gate can never re-derive from a different set of files or a
  // model `room` would have refused to compose (F8). `inputs` may carry transaction-local copies
  // written by room update; only reads use them, so provenance (notably brief.previous) never
  // points at a file that disappears after publication.
  const { inputs, diagnostics } = loadProgram({ manifestDir: MANIFEST_DIR, manifest, program, staged: stagedInputs[program.id] })
  if (diagnostics.length) fail(formatRoomDiagnostic(diagnostics[0], program.id))
  const { paths, repo, issuesSnapshot, model, topo, hasMapDeclared, health, findings, brief } = inputs
  ...
  const docs = stripGateInputs(loadedDocs)
  ...
  // arbiter's milestone projection, opt-in by presence. Loaded through roomload so `check` reads
  // the identical file by the identical rule — a gate grading a different projection than the
  // composer rendered would prove nothing.
  const arbiterMilestones = loadArbiterMilestones(arbiterMilestonesPath(program, repo), (path) => readJson(path, `${label} arbiter milestones`))
  const arbiterUseCases = loadArbiterUseCases(arbiterUseCasesPath(program, repo), (path) => readJson(path, `${label} arbiter use cases`))
  const arbiterRunbooks = loadArbiterRunbooks(arbiterRunbooksPath(program, repo), (path) => readJson(path, `${label} arbiter runbooks`))
  const derived = deriveAll({ repo, model, topo, issuesSnapshot, health, findings, brief, briefPath: paths.brief, manifest: paths.programManifest, gateInputs, arbiterMilestones, arbiterUseCases, arbiterRunbooks, docs, hasMapDeclared }, deriveContext)
  return { ...program, repo, issuesSnapshot, model, topo, health, findings, brief, hasMap: hasMapDeclared, derived, deriveContext, docs }
})
...
// The information architecture, injected rather than restated. The viewer mounts exactly the
// lenses lib/lenses.mjs declares and scripts/room-presentable.mjs grades the artifact against the
// same table, so a route can no longer exist in one place and not the other (I20).
const lensesJson = escJs(JSON.stringify(LENSES.map(({ id, route, question }) => ({ id, route, question }))))

let out = readFileSync(join(HERE, 'viewer', 'control-room.html'), 'utf-8')
for (const seam of ['/*__ROOM_JSON__*/null', '/*__STRINGS__*/null', '/*__LENSES__*/null', '/*__WRITABLE__*/false', '<!--__HOLO_SRC__-->']) {
...
out = out.replace('/*__ROOM_JSON__*/null', roomJson).replace('/*__STRINGS__*/null', stringsJson)
  .replace('/*__LENSES__*/null', lensesJson)
