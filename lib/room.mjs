...
const loadedPrograms = active.map((program) => {
  ...
  const { inputs, errors } = loadProgram({ manifestDir: MANIFEST_DIR, manifest, program, staged: stagedInputs[program.id] })
  if (errors.length) fail(errors[0])
  const { paths, repo, issuesSnapshot, model, topo, hasMap, health, findings, brief, docs, gateInputs, arbiterMilestones, arbiterUseCases, arbiterRunbooks } = inputs
