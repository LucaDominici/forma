...
const loadedPrograms = active.map((program) => {
  ...
  const health = overlay(paths.health, 'c4-health.schema.json', 'health overlay', { verdicts: [], dependencyConfirmations: [] })
  const findings = overlay(paths.findings, 'c4-findings.schema.json', 'findings overlay', { findings: [] })
  ...
  const brief = paths.brief === null ? null : overlay(paths.brief, 'c4-brief.schema.json', 'brief overlay', { claims: [] })
