...
const loadedPrograms = active.map((program) => {
  ...
  const deriveContext = {}
  const derived = deriveAll({ repo, model, topo, issuesSnapshot, health, findings, manifest: paths.programManifest, gateInputs }, deriveContext)
  return { ...program, repo, issuesSnapshot, model, topo, health, findings, hasMap, derived, deriveContext, docs }
})
