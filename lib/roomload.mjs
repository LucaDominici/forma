...
const schemaUrl = (name) => new URL(`./schema/${name}`, import.meta.url)

// One programme's inputs, resolved and read the same way for `room` (which fails fast) and `check`
// (which collects every error before failing). F8: the two used to carry separate copies of this
// load, and only room's schema-validated the model — a hand-edited model.json that `room` would
// refuse to compose passed `check --room` silently. `errors` is total: nothing here calls
// `process.exit`, so a caller can keep going across programmes (`check`) or stop at the first one
// (`room`, by using `errors[0]`).
export function loadProgram({ manifestDir, manifest, program, staged }) {
  ...
  const readJson = (path, what) => {
    ...
    catch (e) { errors.push(`${label} ${what}: ${path} — ${(e && e.message) || e}`); return null }
  }
  const readValidated = (path, schemaName, what) => {
    const obj = readJson(path, what)
    if (obj === null) return null
    const name = schemaName || 'c4-model.schema.json'
    const schemaErrs = validateModel(obj, schemaName ? schemaUrl(schemaName) : undefined)
    if (schemaErrs.length) { errors.push(`${label} ${what}: ${path} fails ${name}:\n - ` + schemaErrs.join('\n - ')); return null }
    return obj
  }
  // A declared overlay that does not exist yet is the state before the first `audit --apply`:
  // empty, not an error (room.mjs's original comment, unchanged by the move).
  const overlay = (path, schemaName, what, empty) => (path === null || !existsSync(path)) ? empty : readValidated(path, schemaName, what)

  const issuesSnapshot = readValidated(inputs.issues, 'c4-issues.schema.json', 'issue snapshot')
  if (issuesSnapshot) {
    if (issuesSnapshot.truncated === true) errors.push(`${label}: issue snapshot is truncated; refusing to build counts and proportions from incomplete facts.`)
    for (const error of snapshotManifestErrors(program, issuesSnapshot)) errors.push(`${label}: ${error}`)
  }

  if (Boolean(paths.model) !== Boolean(paths.topology)) errors.push(`${label}: model and topology must either both be present or both be absent.`)
  const hasMap = Boolean(paths.model)
  let model = null, topo = null
  if (hasMap) {
    model = readValidated(inputs.model, undefined, 'model')
    topo = readJson(inputs.topology, 'topology')
  }
  ...
  // Opt-in by presence (I11): a programme that declares no brief derives none.
  ...
  const readArbiterJson = (path) => {
    try { return JSON.parse(readFileSync(path, 'utf-8')) }
    catch (e) { throw new Error(`${label}: ${path} — ${(e && e.message) || e}`) }
  }
  const loadProjection = (loader, path) => {
    try { return loader(path, readArbiterJson) } catch (e) { errors.push(`${label} ${(e && e.message) || e}`); return null }
  }
  const arbiterMilestones = loadProjection(loadArbiterMilestones, arbiterMilestonesPath(program, repo))
  const arbiterUseCases = loadProjection(loadArbiterUseCases, arbiterUseCasesPath(program, repo))
  const arbiterRunbooks = loadProjection(loadArbiterRunbooks, arbiterRunbooksPath(program, repo))

  return {
    inputs: {
      paths, repo, issuesSnapshot, model, topo, hasMap, health, findings, brief,
      docs, gateInputs, arbiterMilestones, arbiterUseCases, arbiterRunbooks,
    },
    errors,
  }
}
