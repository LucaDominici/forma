...
export function loadDocs(repo, { include, canon, maxBytes } = {}) {
  ...
  for (const path of wanted) {
    let text
    try { text = readFileSync(join(repo, path), 'utf-8') } catch (e) { listed.push({ path, lines: null, why: String((e && e.message) || e) }); continue }
    ...
    if (bytes + size > limit) { listed.push({ path, lines: text.split('\n').length, why: 'budget' }); continue }
    bytes += size
    embedded.push({ path, title: titleOf(text), lines: text.split('\n').length, bytes: size, text })
  }
