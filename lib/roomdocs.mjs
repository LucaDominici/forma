...
import { resolveEvidencePath } from './evidence.mjs'
import { trackedFiles } from './repofiles.mjs'

const DEFAULT_MAX_BYTES = 400000

// `trackedFiles` returns a Set (git ls-files is already sorted, so insertion order holds); this
// module wants an array to filter and re-sort against globs.
...
// What the viewer is handed: everything loadDocs produced EXCEPT the gate's extraction inputs,
// which are an input to a derivation and never a rendered document. Exported so `room` (which
// composes) and `check` (which re-derives) strip it by the identical rule — the pair of one-liners
// they used to carry separately is exactly the kind of drift roomload.mjs exists to prevent.
export const stripGateInputs = (loadedDocs) =>
  loadedDocs && Object.fromEntries(Object.entries(loadedDocs).filter(([key]) => key !== 'gateInputs'))
