...
export function loadDocs(repo, { include, canon, maxBytes } = {}) {
  ...
  for (const path of wanted) {
    let raw, source
    try { source = confinedSourceRoot(repo, join(repo, path)); raw = readFileSync(source, 'utf-8') } catch (e) { listed.push({ path, lines: null, why: /escapes repository/.test(String((e && e.message) || e)) ? 'outside repository' : String((e && e.message) || e) }); continue }
  ...
  for (const path of rest) {
    let lines = null, why = ''
    try { lines = readFileSync(confinedSourceRoot(repo, join(repo, path)), 'utf-8').split('\n').length } catch (e) { why = /escapes repository/.test(String((e && e.message) || e)) ? 'outside repository' : '' }
    listed.push({ path, lines, why })
  }
  return { embedded, listed, bytes, maxBytes: limit }
}

export const docBytes = (repo, path) => { try { return statSync(confinedSourceRoot(repo, join(repo, path))).size } catch { return null } }
