...
import { classifyVerdictStaleness, codepointCompare, evidenceHashOrNull } from './evidence.mjs'
import { parseArgs } from 'node:util'

// F12: reject an unknown flag instead of silently ignoring it. `--limit` is gone: it was forwarded
// by `room update` but never read here. parseArgs is the ONLY reader of process.argv below, so
// `--key=value` is honoured too.
let values
try {
  ({ values } = parseArgs({ args: process.argv.slice(2), allowPositionals: true, strict: true, options: {
    repo: { type: 'string' }, model: { type: 'string' }, issues: { type: 'string' },
    'gh-cmd': { type: 'string' }, 'gh-repo': { type: 'string' }, 'body-max-bytes': { type: 'string' },
    health: { type: 'string' }, today: { type: 'string' }, 'stale-after-days': { type: 'string' },
    workflow: { type: 'string', multiple: true }, release: { type: 'boolean' },
  } }))
} catch (e) { console.error(`[forma verify] ${e.message}`); process.exit(1) }
const REPO = values.repo || process.cwd()
const MODEL = values.model || join(REPO, 'docs/architecture/c4-model.json')
const ISSUES = values.issues || join(REPO, 'docs/architecture/c4-issues.json')
const GH_CMD = values['gh-cmd'] || 'gh' // split on whitespace; tests point it at a stub
const BODY_MAX_BYTES = Number(values['body-max-bytes'] ?? '65536')
const HEALTH = values.health ?? null
const TODAY = values.today ?? null
const STALE_AFTER_DAYS = Number(values['stale-after-days'] ?? '14')
...
const GH_REPO = values['gh-repo'] ?? (model && model.meta && model.meta.ghRepo) ?? null
if (!/^[^/\s]+\/[^/\s]+$/.test(String(GH_REPO || ''))) fail('no target repo: pass --gh-repo <owner/repo>, or set meta.ghRepo in the topology.')
const WORKFLOWS = (values.workflow || []).map((value) => {
...
// Only the GraphQL "this field doesn't exist / isn't accessible on this schema" class means
// "this API has no dependency fields" — every other failure (auth, network, rate limit) is a
// transport failure that says nothing about schema support, and must not silently retry without
// dependencies (F5, audit 2026-09-14): that would write a snapshot with `dependencies.supported:
// false`, nulling `criticalPath` and `blocked` totals for a reason that was never measured.
// Field-specific AND schema-error-shaped: "Resource not accessible by integration" (an
// auth/permission failure) matches the wording alone but names no field, and must fail loud
// instead of being read as "this API has no dependency fields" (S2 round-1 HIGH-2).
const isUnsupportedFieldError = (text) => /\b(blockedBy|blocking)\b/.test(text)
  && /\b(doesn'?t exist|does not exist|isn'?t accessible|is not accessible|not supported)\b/i.test(text)
let pages, dependenciesSupported = true, dependencyReason = ''
try {
  pages = fetchPages(true)
} catch (e) {
  const message = errorText(e)
  if (!isUnsupportedFieldError(message)) fail(`dependency query failed: ${message}; model (if present) and c4-issues.json left untouched.`)
  dependencyReason = `GitHub dependency fields unavailable: ${message}`
...
// Codepoint order, not a locale-aware compare: ICU ignores the NUL separator edgeKey uses (two
// keys differing only by an embedded NUL compare equal under it), which collapsed distinct edge
// keys to the same sort rank and fell back to insertion order (F2).
const edges = [...nativeEdges.values(), ...proseEdges.values()].sort((a, b) => codepointCompare(edgeKey(a.from, a.to), edgeKey(b.from, b.to)))
...
// A declared health overlay that does not exist yet (before the first `audit --apply`) carries no
// verdicts: nothing to count as stale. An existing one that cannot be parsed is still fatal.
if (HEALTH && existsSync(HEALTH)) {
  let health
  ...
  for (const verdict of verdicts) {
    ...
    const evidenceHash = evidenceHashOrNull(REPO, verdict.evidence, snapshot, `#${verdict.n}`)
