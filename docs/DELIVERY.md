...
| The shipped JavaScript entry points parse. | `npm run lint` | This passes on the branch and checks 35 files through `node --check`. |
...
| The 1.1.0 release candidate is locally installable with its frozen schema contract. | Full suite, `npm pack`, install the tarball into an empty temporary prefix, then `forma --version` | The suite rejects schema major 2 while accepting 1.x; the packaged `forma-arch-1.1.0.tgz` contains 41 files and its installed CLI reports `1.1.0`. Publishing remains the separately authorized remote step. |
