...
| The npm selection is the reviewed runtime surface and clean of editor residue. | `npm pack --dry-run --json` | The prepack guard enforces the exact 38-file allowlist, including the Control Room runtime and schemas; an added, missing or editor-residue file fails before packing. |
...
| A failed or ambiguous refresh cannot publish a mixed Control Room. | Make a later programme fail, or make two programme outputs resolve to the same path | `room update` stages the whole portfolio, rejects duplicate IDs/canonical write targets before work, and rolls snapshots, models, overlays, plans and HTML back together. |
| The embedded C4 drill works without a mouse or microscopic mobile scaling. | Traverse nodes/relationships and the paged table twin at 390 px | SVG nodes expose category, state and evidence in their accessible names; relationships are focusable; detail focus returns on Escape; the canvas pans natively; the bounded screen table carries the same node/relationship facts. |
...
| A counter-verdict reaches the same pill and gate as every other health fact. | `forma audit --apply <counter.json> --counter-plan <plan.json>`, then `forma room` and `forma check` | A contradiction becomes a `bad` issue verdict with the agent's reason plus a durable finding; unsupported becomes a warning. Apply rejects unresolved evidence before either overlay write, and `check` independently rejects a later-broken health or finding anchor. |
...
| Control Room work is bounded at real programme volume. | Required `browser` CI job over dogfood plus the 2,500-issue stress fixture | `scripts/browser-gate.mjs` measures all seven routes at 3440/1920/1366/390, Axe A/AA, keyboard map/table behavior, 44 px targets, bounded issue rows and an exact seven-page static print projection; the report and PDFs are retained as a CI artifact. |
...
| Control Room locale keys | 241 | At en/it parity, and every one read by the template — both halves asserted by `npm test`. |
...
| The documented shipped-file count used to be unenforced. | `scripts/check-clean.mjs` now compares every selected runtime path with the reviewed 38-file set before `npm pack`. | Resolved: a future selection change is red until the allowlist is deliberately reviewed and updated. |
