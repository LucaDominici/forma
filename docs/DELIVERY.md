...
| The shipped JavaScript entry points parse. | `npm run lint` | This passes on the branch and checks 36 files through `node --check`. |
...
| The documented shipped-file count was unenforced. | `AGENTS.md` used to record a bare count nothing checked. | Resolved: [`scripts/check-clean.mjs`](../scripts/check-clean.mjs) enforces a 42-entry allowlist at `prepack` and refuses to publish a changed selection; `AGENTS.md` states the same 42. |
