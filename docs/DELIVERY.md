...
| The shipped JavaScript entry points parse. | `npm run lint` | This passes on the branch and checks 50 files through `node --check`. |
...
| The single-model public demo meets its five publication predicates. | `node scripts/presentable.mjs docs/demo/c4-model.json` | The suite grades the shipped model itself, not a cleaned copy ([`test/model.test.mjs:899-918`](../test/model.test.mjs#L899-L918)). |
...
| Control Room work is bounded at real programme volume. | Generate the 2,500-issue stress fixture, run browser/mobile/print checks and `room-presentable` | Only the lenses a programme can answer mount, and they mount lazily — on the stress fixture that is two of six; Queue, Kanban and portfolio Moving use 40-row pages; interactive archives are summarized in print and every address the five-view IA published, `/auto` and `/kanban` included, resolves to the lens that inherited its question. |
...
| The 1.3.0 release candidate is locally installable with its frozen schema contract. | Full suite, `npm pack`, install the tarball into an empty temporary prefix, then `forma --version` | The suite rejects schema major 2 while accepting 1.x; the packaged `forma-arch-1.3.0.tgz` contains 42 files and its installed CLI reports `1.3.0`. Publishing remains the separately authorized remote step. |
...
| The documented shipped-file count was unenforced. | `AGENTS.md` used to record a bare count nothing checked. | Resolved: [`scripts/check-clean.mjs`](../scripts/check-clean.mjs) enforces a 43-entry allowlist at `prepack` and refuses to publish a changed selection; `AGENTS.md` states the same 43. |
