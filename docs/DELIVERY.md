...
| The shipped JavaScript entry points parse. | `npm run lint` | This passes on the branch and checks 44 files through `node --check`. |
...
| The single-model public demo meets its five publication predicates. | `node scripts/presentable.mjs docs/demo/c4-model.json` | The suite grades the shipped model itself, not a cleaned copy ([`test/model.test.mjs:951-966`](../test/model.test.mjs#L951-L966)). |
