...
|---|---|---|---|---|
| D-1 | **Shipped-file count is documented, not enforced.** `AGENTS.md` states the `npm pack` selection size; `scripts/check-clean.mjs` only checks for editor residue. | The number has moved repeatedly (30 → 31 → 36 → 38). A file could enter or leave the package unnoticed. | Any release where the selection changed without a deliberate note. | Either enforce the intended list mechanically in `check-clean.mjs`, or drop the number from prose and let `npm pack --dry-run` be the only statement. Deciding *which* is the open part. |
...
- **D-8 (closed by #53).** The layout floor is now browser-gated outside the runtime dependency
  tree: CI composes the briefing, discovers its published routes, and rejects
  `scrollHeight > innerHeight` at 3440×1440 and 1920×900. The gate also proves its own forced
  overflow control rejects every measured route.
