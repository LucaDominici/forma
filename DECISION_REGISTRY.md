...
Enforcement: `lib/render.mjs` emits the frontmatter block; the scaffold regression in `test/gen.test.mjs` requires all eight fields, so a regression is caught by the public gate.
