...
1. Run `forma audit --repo . --plan docs/architecture/audit-plan.json` with the programme's explicit paths when they differ from the defaults.
...
```json
{"results":[{"claimId":"<exact plan id>","verdict":"holds|contradicted|unsupported","reason":"<one sentence explaining what the evidence proves>","evidence":{"type":"file|commit|gh","ref":"<resolvable reference>"}}]}
```
