...
For `brief-claim` results the verdict lands ON the claim with today's date (`verified{verdict, reason, evidence, at, engine}`): a fresh `holds` from an engine DIFFERENT than the one that wrote the claim (`author.engine`) is the only thing that lets the dashboard colour that claim — the same engine holding its own claim, or either engine being unrecorded, is kept on the claim but rendered "self-held", never coloured. `contradicted` and `unsupported` also become findings.

5. Apply the result with `forma audit --repo . --today YYYY-MM-DD --stale-after-days N --apply docs/architecture/audit-counter.json --counter-plan docs/architecture/audit-plan.json --engine codex`. `--engine codex` is what stamps `verified.engine` on every verdict this apply writes — always pass it; omitting it makes every hold you grant land as "self-held" regardless of who actually wrote the claim. Forma rejects a changed plan outright; inside a current plan it applies results one by one, refuses any entry whose evidence does not resolve, and names every refusal in `c4-health.json` → `lastApply.rejected`. Claims you leave unanswered are listed as `unanswered` and simply keep no fresh verdict — say what you could not verify rather than inventing an anchor.

For an unattended portfolio, let the scheduler run `forma room update` first, then — when a writing agent runs the `forma-room-update` ritual — its fill lands via `forma room update --skip-verify --fill --author-engine <the engine that wrote the fill>`. Run this workflow for every active programme over the plan THAT COMMAND regenerated, then run `forma room update --skip-verify --counter --verifier-engine codex`. `--fill` and `--counter` never run in the same `room update` call: `--fill` re-plans from the current state before applying, so a counter result written against any earlier plan is stale the moment that re-plan happens — `room update` refuses the combination by name rather than failing on a stale planHash. `--verifier-engine` on this last, counter-only call is what stamps `verified.engine`; omitting it silently overwrites it back to unknown and turns every hold this run granted into "self-held" the moment anything re-applies over it. Each programme must declare `health`, `findings`, `auditPlan`, `counterResults` (and `auditFill`, `brief.path` when those channels are in play) in the manifest. Keep the manifest's `today` unchanged for the whole run: the plan's identity does not depend on the day, but the verdict's date does. Each update regenerates the plan and fails if any result is missing or stale.
